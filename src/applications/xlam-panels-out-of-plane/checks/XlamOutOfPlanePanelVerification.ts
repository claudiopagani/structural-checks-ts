// Mechanical TypeScript migration from strutture-js 6f33baead8b88166c4b2cf94af41763412e3c751; source path: src/applications/xlam-panels-out-of-plane/checks/XlamOutOfPlanePanelVerification.js.
/* eslint-disable @typescript-eslint/ban-ts-comment, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-return, @typescript-eslint/no-unused-vars */
// @ts-nocheck
...
export class XlamOutOfPlanePanelVerification {
  verify(model) {
    ...
    for (const layer of section.layers) {
      ...
      const tau = (vEd * staticMoment) / (bendingStiffness * section.effectiveWidth);
    ...
    const deflectionShort =
      (5 * (qSle || qPermanent + qVariable) * span ** 4) / (384 * stiffness.bendingStiffness) +
    ...
    const checks = [
      ...
      },
      {
        ...
        ...evaluateCheck(Math.abs(deflectionLong), span / longTermDeflectionLimitDenominator),
      },
    ];
