...
import type { MomentCurvaturePostUltimateResponse } from "./types.js";
...
const SUPPORTED_POST_ULTIMATE_RESPONSES = new Set<MomentCurvaturePostUltimateResponse>([
  "retain",
  "linear-softening",
  "zero-stress",
]);

export interface MomentCurvaturePostUltimateOptions {
...
export interface ResolvedMomentCurvaturePostUltimateOptions {
...
export function resolvePostUltimateOptions({
...
}: MomentCurvaturePostUltimateOptions = {}): ResolvedMomentCurvaturePostUltimateOptions {
  const resolvedPostUltimateMomentDrop =
    postUltimateMomentDrop ?? postPeakMomentDrop ?? DEFAULT_POST_ULTIMATE_MOMENT_DROP;
  ...
  if (
    ...
    throw new Error("RCMomentCurvatureAnalyzer postUltimateMomentDrop must be between 0 and 1.");
  }

  if (!Number.isFinite(maxPostUltimateCurvatureRatio) || maxPostUltimateCurvatureRatio <= 1) {
    throw new Error(
  ...
  if (!SUPPORTED_POST_ULTIMATE_RESPONSES.has(postUltimateResponse)) {
    throw new Error(`Unsupported RC post-ultimate response: ${postUltimateResponse}.`);
  }

  if (!Number.isFinite(postPeakCurvatureGrowthFactor) || postPeakCurvatureGrowthFactor <= 1) {
    throw new Error(
  ...
  if (!Number.isInteger(maxPostPeakPoints) || maxPostPeakPoints < 1) {
    throw new Error("RCMomentCurvatureAnalyzer maxPostPeakPoints must be a positive integer.");
  }
