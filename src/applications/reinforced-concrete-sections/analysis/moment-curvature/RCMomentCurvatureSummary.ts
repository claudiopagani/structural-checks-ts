import { projectedMoment, round, summarizeStateCheck } from "./RCMomentCurvaturePolicies.js";
import type { MomentCurvatureDuctility, MomentCurvaturePoint } from "./types.js";

export function summarizeMomentCurvaturePoint(
  ...
  return {
    ...
    Mx: round(point.Mx, 6),
    My: round(point.My, 6),
    projectedMoment: round(point.projectedMoment ?? projectedMoment(point), 6),
    ...
    postUltimateState:
      point.postUltimateState == null
        ? null
        : {
            referenceMoment: round(point.postUltimateState.referenceMoment, 6),
            reference: point.postUltimateState.reference,
            targetMoment: round(point.postUltimateState.targetMoment, 6),
            moment: round(point.postUltimateState.moment, 6),
            targetDropRatio: round(point.postUltimateState.targetDropRatio, 6),
            actualDropRatio: round(point.postUltimateState.actualDropRatio, 6),
    ...
    postPeakState:
      point.postPeakState == null
        ? null
        : {
            maximumMoment: round(point.postPeakState.maximumMoment, 6),
            targetMoment: round(point.postPeakState.targetMoment, 6),
            moment: round(point.postPeakState.moment, 6),
            targetDropRatio: round(point.postPeakState.targetDropRatio, 6),
            actualDropRatio: round(point.postPeakState.actualDropRatio, 6),
    ...
    extremes: {
      ...
      maxSteelTensionStrain:
        point.state.extremes.maxSteelTensionStrain == null
          ? null
          : {
              value: round(point.state.extremes.maxSteelTensionStrain.stress, 6),
              strain: round(point.state.extremes.maxSteelTensionStrain.strain),
      ...
      maxSteelCompressionStrain:
        point.state.extremes.maxSteelCompressionStrain == null
          ? null
          : {
              value: round(point.state.extremes.maxSteelCompressionStrain.stress, 6),
              strain: round(point.state.extremes.maxSteelCompressionStrain.strain),
    ...
    balancedFailureState:
      point.balancedFailureState == null
        ...
        : {
            ...
            concrete: summarizeStateCheck(point.balancedFailureState.concrete),
            steel: summarizeStateCheck(point.balancedFailureState.steel),
            effectiveDepth: round(point.balancedFailureState.effectiveDepth, 6),
            neutralAxisDepth: round(point.balancedFailureState.neutralAxisDepth, 6),
            compressedEdgeProjection: round(point.balancedFailureState.compressedEdgeProjection, 6),
            tensionReinforcementProjection: round(
              point.balancedFailureState.tensionReinforcementProjection,
              6,
            ),
            compressedEdgeY: round(point.balancedFailureState.compressedEdgeY, 6),
            tensionReinforcementY: round(point.balancedFailureState.tensionReinforcementY, 6),
            assignedAxialForce: round(point.balancedFailureState.assignedAxialForce, 6),
            balancedAxialForce: round(point.balancedFailureState.balancedAxialForce, 6),
            axialResidual: round(point.balancedFailureState.axialResidual, 6),
...
export function summarizeMomentCurvatureDuctility(
  ...
  return {
    ...
    ultimateMomentDropRatio: round(ductility.ultimateMomentDropRatio, 6),
    ...
    momentDropPoint: summarizeInterpolatedPoint(ductility.momentDropPoint),
    materialUltimatePoint: summarizeInterpolatedPoint(ductility.materialUltimatePoint),
    ultimatePoint: summarizeInterpolatedPoint(ductility.ultimatePoint),
  };
}

function summarizeInterpolatedPoint(
  point: MomentCurvatureDuctility["ultimatePoint"],
): Record<string, unknown> | null {
  if (point == null) {
    return null;
  }
