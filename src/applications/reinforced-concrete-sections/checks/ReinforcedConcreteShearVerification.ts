...
import { COSENZA_METHOD, DEFAULT_RC_SHEAR_UNITS } from "./shear/shearUtils.js";
import {
...
export { computeWithTransverseResistanceAtCotTheta };
...
export class ReinforcedConcreteShearVerification {
  ...
  method: string | null;
  shear: RcShearInput;
  section: RcShearVerificationOptions["section"];
  concreteMaterial: RcShearVerificationOptions["concreteMaterial"];
  reinforcementMaterial: RcShearVerificationOptions["reinforcementMaterial"];
  ...
  verifySectionActions({
  ...
  }: RcShearSectionActionInput = {}): RcShearVerificationData {
    const configuredMode = shear.mode ?? this.mode ?? this.shear.mode;
    ...
    const resolvedShear: RcShearInput = {
      ...this.shear,
      ...shear,
      ...(configuredMode === undefined ? {} : { mode: configuredMode }),
      ...(configuredMethod === undefined ? {} : { method: configuredMethod }),
    };
    ...
    if (!method) {
      baseWarnings.push("Unsupported RC shear method; use ntc2018 or cosenza-et-al-2016.");
    }
    ...
    let result: RcShearVerificationData;
  ...
  verify(input: RcShearVerificationInput = {}): VerificationResult {
    const section = (input.section === undefined ? this.section : input.section) ?? null;
    const concreteMaterial =
      (input.concreteMaterial === undefined
        ? (this.concreteMaterial ??
          (section?.concreteMaterial as RcShearVerificationInput["concreteMaterial"]))
        : input.concreteMaterial) ?? null;
    const reinforcementMaterial =
      (input.reinforcementMaterial === undefined
        ? (this.reinforcementMaterial ??
          (section?.reinforcementMaterial as RcShearVerificationInput["reinforcementMaterial"]))
        : input.reinforcementMaterial) ?? null;
    const shear = input.shear ?? this.shear;
    const actions = input.actions ?? {};
    const units = input.units ?? resolveUnits(section, { shear });
