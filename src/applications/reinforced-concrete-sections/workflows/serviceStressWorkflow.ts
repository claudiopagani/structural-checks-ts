...
import { resolveServiceStressSolverActions } from "../shared/rcCommon.js";
import { solveRcServiceSectionState } from "../shared/solveRcServiceSectionState.js";

export function runServiceStressWorkflow(
  ...
  const reinforcementMaterial =
    model.materials.reinforcementMaterial ?? context.section.reinforcementMaterial;

  if (reinforcementMaterial == null) {
    throw new Error(
      "ReinforcedConcreteSectionVerification requires a reinforcement material for service-stress.",
    );
  }

  const solved = solveRcServiceSectionState({
    section: context.section,
    reinforcementMaterial: reinforcementMaterial as SteelMaterial,
