// Mechanical TypeScript migration from strutture-js 6f33baead8b88166c4b2cf94af41763412e3c751; source path: src/applications/reinforced-concrete-beam-column-joints/ReinforcedConcreteBeamColumnJointModel.js.
...
function ratios(input: any = {}): Record<string, any> {
...
function normalizeAnchor(input: any, resolver: any, label: string): any {
  ...
  return {
    ...
    designSteelStress:
      input.designSteelStress == null
        ? null
        : positive(resolver.stress(Number(input.designSteelStress)), `${label}.designSteelStress`),
    fctd:
      input.fctd == null ? null : positive(resolver.stress(Number(input.fctd)), `${label}.fctd`),
  };
}

export interface ReinforcedConcreteBeamColumnJointModelOptions {
  [key: string]: any;
}
...
export class ReinforcedConcreteBeamColumnJointModel {
  ...
  constructor({
    ...
    units = null,
    metadata = {},
  }: ReinforcedConcreteBeamColumnJointModelOptions = {}) {
    if (!id) {
    ...
    if (!concreteMaterial || !reinforcementMaterial || !transverseReinforcementMaterial) {
      throw new Error(
    ...
    const beamWidth = positive(resolver.length(Number(geometry.beamWidth)), "geometry.beamWidth");
    ...
    if (columnLongitudinalLayerDistance >= columnDepth) {
      throw new Error("geometry.columnLongitudinalLayerDistance must be smaller than columnDepth.");
    }

    if (beamLongitudinalLayerDistance >= beamHeight) {
      throw new Error("geometry.beamLongitudinalLayerDistance must be smaller than beamHeight.");
    }
    ...
    this.jointHoops = {
      diameter: positive(resolver.length(Number(jointHoops.diameter)), "jointHoops.diameter"),
      totalArea: positive(resolver.area(Number(jointHoops.totalArea)), "jointHoops.totalArea"),
      areaPerSet: positive(resolver.area(Number(jointHoops.areaPerSet)), "jointHoops.areaPerSet"),
      spacing: positive(resolver.length(Number(jointHoops.spacing)), "jointHoops.spacing"),
    };
    this.confinement = {
      faceCoverageRatios: ratios(confinement.faceCoverageRatios),
      oppositeBeamOverlapRatios: ratios(confinement.oppositeBeamOverlapRatios),
      adjacentColumnHoops: {
        controllingAreaPerSet: positive(
          resolver.area(Number(confinement.adjacentColumnHoops?.controllingAreaPerSet)),
          "confinement.adjacentColumnHoops.controllingAreaPerSet",
        ),
        controllingSpacing: positive(
          resolver.length(Number(confinement.adjacentColumnHoops?.controllingSpacing)),
    ...
    this.capacityHierarchy = hierarchyExempt
      ...
      : {
          ...
          effectiveColumnMomentResistance: positive(
            resolver.moment(Number(capacityHierarchy.effectiveColumnMomentResistance)),
    ...
    this.anchorage = {
      ...
      bottomBars: normalizeAnchor(anchorage.bottomBars, resolver, "anchorage.bottomBars"),
    };
    ...
    this.eccentricity = {
      beamAxisOffset,
      transferLeverArm:
        eccentricity.transferLeverArm == null
          ? null
          : positive(
      ...
      reinforcementArea:
        eccentricity.reinforcementArea == null
          ? 0
          : nonNegative(
