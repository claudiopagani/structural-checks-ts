...
import type { ReinforcedConcreteColumnModelOptions } from "./types.js";

export interface ReinforcedConcreteColumnApplicationInput {
  model?: ReinforcedConcreteColumnModel | ReinforcedConcreteColumnModelOptions;
  code?: string;
...
export class ReinforcedConcreteColumnApplication extends StructuralApplication {
  ...
  override run(input: ReinforcedConcreteColumnApplicationInput = {}): VerificationResult {
    if (!input.model) {
      throw new Error("ReinforcedConcreteColumnApplication requires a model.");
    }

    const model =
      input.model instanceof ReinforcedConcreteColumnModel
        ? input.model
