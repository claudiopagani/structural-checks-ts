...
function detailingMetadata(metadata: Record<string, unknown> = {}): Record<string, unknown> {
  ...
  const normativeReference =
    legacyReference === "NTC2018-4.1.2.3.10"
      ? NTC2018_RC_CHAPTER_4_REFERENCES.anchorage
      : legacyReference.startsWith("NTC2018-4.1")
        ? NTC2018_RC_CHAPTER_4_REFERENCES.columnDetailing
        : legacyReference === "NTC2018-7.4.6.1.2"
          ? NTC2018_RC_CHAPTER_7_4_REFERENCES.columnGeometry
          : legacyReference.startsWith("NTC2018-7.4")
            ? NTC2018_RC_CHAPTER_7_4_REFERENCES.columnDetailing
            : null;
...
function ductilityClass(value: string | null | undefined): "CDA" | "CDB" {
...
export class ReinforcedConcreteColumnDetailingVerification {
  ...
  constructor({ code = "NTC2018" }: RcColumnDetailingVerificationOptions = {}) {
    this.code = code;
  }
  ...
  verify({
    model,
    compression = 0,
    normalizedAxialForce = 0,
  }: RcColumnDetailingVerificationInput): VerificationResult {
    ...
    positive(Number(model.reinforcementMaterial?.fyk), "reinforcement fyk");
    const fywd = positive(
      Number(transverse.designStrength ?? model.reinforcementMaterial?.fyd),
      "transverse design strength",
    );

    // NTC 2018 § 4.1.6.1.2, formula [4.1.46].
    ...
    const checks: UtilizationCheck[] = [
      minCheck(
        "rc-column-minimum-longitudinal-area",
        "Minimum column longitudinal reinforcement",
        minimumArea,
        reinforcementArea,
        { reference: "NTC2018-4.1.46" },
      ),
      maxCheck(
        "rc-column-maximum-longitudinal-area",
        "Maximum column longitudinal reinforcement outside lap zones",
        reinforcementArea,
        maximumArea,
        { reference: "NTC2018-4.1.6.1.2" },
      ),
      maxCheck(
        "rc-column-maximum-longitudinal-spacing",
        "Maximum column longitudinal bar spacing",
        maxBarSpacing,
        300,
        { reference: "NTC2018-4.1.6.1.2" },
      ),
      minCheck(
        "rc-column-minimum-longitudinal-diameter",
        "Minimum column longitudinal bar diameter",
        12,
        minBarDiameter,
        { reference: "NTC2018-4.1.6.1.2" },
      ),
      maxCheck(
        "rc-column-ordinary-hoop-spacing",
        "Ordinary column hoop spacing",
        hoopSpacing,
        ordinaryHoopSpacing,
        { reference: "NTC2018-4.1.6.1.2" },
      ),
      minCheck(
        "rc-column-ordinary-hoop-diameter",
        "Ordinary column hoop diameter",
        ordinaryHoopDiameter,
        hoopDiameter,
        { reference: "NTC2018-4.1.6.1.2" },
      ),
    ];
    ...
    if (detailing.seismic?.enabled === true) {
      ...
      const criticalZoneLength =
        clearHeight < 3 * sectionDepthInBending
          ? clearHeight
      ...
      const alphaN = Math.max(
        0,
        1 -
          restrainedSpacings.reduce<number>((sum, value) => sum + Number(value) ** 2, 0) /
            (6 * coreWidth * coreDepth),
      );
      const alphaS =
        Math.max(0, 1 - hoopSpacing / (2 * coreWidth)) *
      ...
      const concreteFcd = positive(Number(model.concreteMaterial?.fcd), "concrete fcd");
      const omegaWd = (volumetricRatio * fywd) / concreteFcd;
      const epsilonSyD =
        fyd /
        positive(
      ...
      const ductilityDemand = Math.max(
        ...
        30 *
          ...
          (Math.min(width, depth) / Math.min(coreWidth, coreDepth)) -
          0.035,
      );
      ...
      checks.push(
        minCheck(
          "rc-column-seismic-minimum-dimension",
          "Minimum dissipative column dimension",
          250,
          Math.min(width, depth),
          { reference: "NTC2018-7.4.6.1.2" },
        ),
        minCheck(
          "rc-column-seismic-longitudinal-ratio-min",
          "Minimum seismic longitudinal reinforcement ratio",
          0.01,
          longitudinalRatio,
          { reference: "NTC2018-7.4.28" },
        ),
        maxCheck(
          "rc-column-seismic-longitudinal-ratio-max",
          "Maximum seismic longitudinal reinforcement ratio",
          longitudinalRatio,
          0.04,
          { reference: "NTC2018-7.4.28" },
        ),
        maxCheck(
          "rc-column-seismic-longitudinal-spacing",
          "Maximum seismic longitudinal bar spacing",
          maxBarSpacing,
          250,
          { reference: "NTC2018-7.4.6.2.2" },
        ),
        minCheck(
          "rc-column-seismic-hoop-diameter",
          "Minimum confinement hoop diameter",
          seismicHoopDiameter,
          hoopDiameter,
          { reference: "NTC2018-7.4.6.2.2" },
        ),
        maxCheck(
          "rc-column-seismic-hoop-spacing",
          "Maximum confinement hoop spacing",
          hoopSpacing,
          seismicHoopSpacing,
          { reference: "NTC2018-7.4.6.2.2" },
        ),
        maxCheck(
          "rc-column-seismic-restrained-bar-spacing",
          "Maximum spacing between laterally restrained bars",
          Math.max(...restrainedSpacings.map(Number)),
          restrainedSpacingLimit,
          { reference: "NTC2018-7.4.6.2.2" },
        ),
        minCheck(
          "rc-column-seismic-omega-wd",
          "Minimum mechanical confinement ratio",
          omegaMinimum,
          omegaWd,
          { reference: "NTC2018-7.4.30" },
        ),
        minCheck(
          "rc-column-seismic-ductility-confinement",
          "Confinement for curvature ductility demand",
          ductilityDemand,
          alpha * omegaWd,
          { reference: "NTC2018-7.4.29-7.4.31" },
        ),
      );
      seismicOutputs = {
        ductilityClass: dc,
    ...
    let anchorageOutput: Record<string, unknown> | null = null;
    if (detailing.anchorage) {
      const anchor = detailing.anchorage;
      const concreteFctm = Number(model.concreteMaterial?.fctm);
      const gammaC = Number(model.concreteMaterial?.metadata.gammaC);
      const fctd = positive(Number(anchor.fctd ?? (0.7 * concreteFctm) / gammaC), "anchorage.fctd");
      const bond = calculateEn1992DesignBondStrength({
        fctd,
        barDiameter: Number(anchor.barDiameter),
      ...
      checks.push(
        minCheck(
          "rc-column-longitudinal-anchorage",
          "Column longitudinal bar anchorage",
          requiredLength,
          Number(anchor.availableLength),
          withNormativeReferences(
            {
              reference: axialTension ? "NTC2018-7.4.6.2.2" : "NTC2018-4.1.2.3.10",
              fbd: round(bond.fbd),
              axialTensionFactor: axialTension ? 1.5 : 1,
            },
            [
              EN1992_RC_EXTERNAL_REFERENCES.bondAndAnchorage,
              NTC2018_RC_CHAPTER_4_REFERENCES.anchorage,
              NTC2018_RC_CHAPTER_4_REFERENCES.bondDesignStrength,
              ...(axialTension ? [NTC2018_RC_CHAPTER_7_4_REFERENCES.columnDetailing] : []),
            ],
          ),
        ),
      );
    ...
    return new VerificationResult({
      ...
      metadata: withNormativeReferences(
        ...
        [
          NTC2018_RC_CHAPTER_4_REFERENCES.columnDetailing,
          ...(detailing.seismic?.enabled === true
            ? [
                NTC2018_RC_CHAPTER_7_4_REFERENCES.columnGeometry,
                NTC2018_RC_CHAPTER_7_4_REFERENCES.columnDetailing,
              ]
