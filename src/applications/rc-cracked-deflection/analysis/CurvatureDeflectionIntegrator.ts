// Mechanical TypeScript migration from strutture-js 6f33baead8b88166c4b2cf94af41763412e3c751; source path: src/applications/rc-cracked-deflection/analysis/CurvatureDeflectionIntegrator.js.
...
function interpolateRawDeflection(points: any[], rawDeflections: any[], x: number): number {
...
function interpolateCompatibleDisplacements(points: any[], samples: any): any[] | null {
  const sorted = (samples ?? [])
    .filter((sample: any) => Number.isFinite(sample.x) && Number.isFinite(sample.deflection))
    .sort((first: any, second: any) => first.x - second.x);
...
export function createNaturalCubicSpline(
  xs: any[],
  ys: any[],
): { evaluate: (x: any) => { value: any; slope: any } } {
  ...
  const lower: any[] = new Array(size - 2).fill(0);
  const diagonal: any[] = new Array(size - 2).fill(0);
  const upper: any[] = new Array(size - 2).fill(0);
  const rhs: any[] = new Array(size - 2).fill(0);
  ...
  const secondDerivatives: any[] = new Array(size).fill(0);
  secondDerivatives[size - 2] = rhs[size - 3] / diagonal[size - 3];
  for (let index = size - 4; index >= 0; index -= 1) {
    secondDerivatives[index + 1] =
      (rhs[index] - upper[index] * secondDerivatives[index + 2]) / diagonal[index];
  }

  const intervalIndex = (x: any) => {
    if (x <= xs[0]) return 0;
  ...
  return {
    evaluate(x) {
      ...
      const value =
        a * ys[index] +
        b * ys[index + 1] +
        (((a ** 3 - a) * secondDerivatives[index] + (b ** 3 - b) * secondDerivatives[index + 1]) *
          span ** 2) /
          6;
      const slope =
...
function linearSupportCorrection(
  points: any[],
  rotations: any[],
  rawDeflections: any[],
  x0: number,
...
function smoothSupportCorrection(
  ...
  const correctionSpline = createNaturalCubicSpline(supportStations, correctionValues);
...
export function integrateCurvature(
  points: any[],
  supports: any[] = [],
  { displacementSamples = null }: { displacementSamples?: any[] | null } = {},
): any[] {
  if (points.length < 2) {
    return points.map((point) => ({
      ...point,
  ...
  const rotations: any[] = [0];
  const rawDeflections: any[] = [0];
  ...
  const compatibleDisplacements = interpolateCompatibleDisplacements(points, displacementSamples);
  ...
  const supportStations = [
    ...new Set(
      supports
        .filter((support: any) => support.restraints?.uy)
        .map((support: any) => (Number.isFinite(support.station) ? support.station : null))
  ...
  if (supportStations.length > 2) {
    return smoothSupportCorrection(points, rotations, rawDeflections, supportStations);
  }
