// Mechanical TypeScript migration from strutture-js 6f33baead8b88166c4b2cf94af41763412e3c751; source path: src/applications/rc-cracked-deflection/analysis/HyperstaticDeflectionIteration.js.
...
import { SingleBeamModel, loadsForCombination } from "../../../domain/beams/SingleBeamInput.js";
...
export interface HyperstaticDeflectionIterationOptions {
  femBuilder?: any;
...
export interface HyperstaticDeflectionIterationInput {
  model: any;
  combination: any;
  curve: any;
  curveResolver?: ((input: any) => any) | null;
}
...
export class HyperstaticDeflectionIteration {
  public femBuilder: any;
  public relaxationFactor: number;
  ...
  constructor({
    ...
    if (!Number.isFinite(relaxationFactor) || relaxationFactor <= 0 || relaxationFactor > 1) {
      throw new Error("HyperstaticDeflectionIteration relaxationFactor must be in (0, 1].");
    }

    if (!Number.isFinite(tolerance) || tolerance <= 0) {
      throw new Error("HyperstaticDeflectionIteration tolerance must be positive.");
    }

    if (!Number.isInteger(maxIterations) || maxIterations <= 0) {
      throw new Error("HyperstaticDeflectionIteration maxIterations must be a positive integer.");
    }
  ...
  iterate({
    ...
    curve,
    curveResolver = null,
  }: HyperstaticDeflectionIterationInput): any {
    ...
    const finalizePerformance = () => {
      ...
      performance.curveLookupCount =
        (curve?.lookupCount ?? curveLookupCountAtStart) - curveLookupCountAtStart;
    ...
    if (!curve) {
      throw new Error("HyperstaticDeflectionIteration requires a SectionMomentCurvatureCurve.");
    }
    ...
    const femModel: any = this.femBuilder.build(beamModel, {
      loads: factoredLoads,
      context: comboContext,
    });

    // 2. Initial EI values from the model's elements (gross).
    const elements: any[] = femModel.elements ?? [];
    const elementLoadIndex: any = createElementLoadIndex(femModel.loads ?? []);
    const initialEI = elements.map((el) => el.flexuralRigidity);
    const currentEI = [...initialEI];
    ...
    let solution: any = solveFem();
    let previousActions: any[] = this._extractMidActions(femModel, solution, elementLoadIndex);
    ...
    for (let iter = 0; iter < this.maxIterations; iter += 1) {
      ...
      const stiffnessResidual = targetEI.map((value, index) => value - currentEI[index]);
      if (previousStiffnessResidual) {
        const previousResidual = previousStiffnessResidual;
        const residualDifference = stiffnessResidual.map(
          (value, index) => value - (previousResidual[index] ?? 0),
        );
        const denominator = residualDifference.reduce((sum, value) => sum + value ** 2, 0);
        const numerator = previousResidual.reduce(
          (sum, value, index) => sum + value * (residualDifference[index] ?? 0),
          0,
        );
        const aitkenFactor =
          denominator > 0
        ...
        if (Number.isFinite(aitkenFactor) && aitkenFactor > 0) {
          const nextFactor = Math.min(this.relaxationFactor, Math.max(0.05, aitkenFactor));
      ...
      for (let i = 0; i < elements.length; i += 1) {
        const newEI =
          effectiveRelaxationFactor * targetEI[i] + (1 - effectiveRelaxationFactor) * currentEI[i];
        const relChange =
          currentEI[i] !== 0 ? Math.abs(newEI - currentEI[i]) / Math.abs(currentEI[i]) : 0;
      ...
      solution = solveFem();
      const newActions = this._extractMidActions(femModel, solution, elementLoadIndex);
    ...
    const result: any = sampleBeamResult({
      model: beamModel,
      femModel,
      solution,
  ...
  _extractMidMoments(
    femModel: any,
    solution: any,
    elementLoadIndex: any = createElementLoadIndex(femModel.loads ?? []),
  ): any[] {
    return this._extractMidActions(femModel, solution, elementLoadIndex).map((action) => action.m);
  }
  ...
  _extractMidActions(
    ...
    return (femModel.elements ?? []).map((element: any) => {
  ...
  _relativeChange(oldValues: any[], newValues: any[]): number {
  ...
  _sampleCompatibleDisplacements({ model, femModel, solution }: any): any[] {
    ...
    for (const element of femModel.elements ?? []) {
      ...
      const localDisplacements: any[] = element.localDisplacements(
