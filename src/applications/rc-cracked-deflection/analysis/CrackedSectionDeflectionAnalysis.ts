// Mechanical TypeScript migration from strutture-js 6f33baead8b88166c4b2cf94af41763412e3c751; source path: src/applications/rc-cracked-deflection/analysis/CrackedSectionDeflectionAnalysis.js.
...
import { slendernessCheck, utilizationCheck } from "./DeflectionChecks.js";
import {
...
export interface CrackedSectionDeflectionAnalysisOptions {
  code?: string;
  metadata?: Record<string, unknown>;
}

export interface CrackedSectionDeflectionAnalyzeInput {
  beamId?: string | null;
  analysisResult?: any;
  section?: any;
  concreteMaterial?: any;
  reinforcementMaterial?: any;
  serviceability?: any;
  mesh?: any;
  solver?: any;
...
function deflectionMetadata(metadata: any = {}, includeShrinkage = false) {
  return withNormativeReferences(metadata, [
    ...
    ...(includeShrinkage ? [EN1992_RC_EXTERNAL_REFERENCES.shrinkageCurvature] : []),
  ]);
}

function isQuasiPermanent(type: any) {
  return normalizeCombinationType(type) === "SLE_QUASI_PERMANENT";
}

function resolvePerformanceProfile(name: any) {
  if (name == null || name === false) {
    return null;
  }

  return (RC_DEFLECTION_PERFORMANCE_PROFILES as Record<string, any>)[String(name)] ?? null;
}
...
function resolveAnalysisOptions({
  ...
  const resolvedOutput = {
    ...
    maxPointsPerCombination:
      ...
      null,
    includePointDetails:
      output?.includePointDetails ?? serviceability.deflection?.includePointDetails ?? false,
  };
...
function numericCacheKey(value: any) {
  return Number.isFinite(value) ? value.toPrecision(12) : String(value);
}

function quantizeAxialForce(value: any, tolerance: any) {
...
function transformedGrossInertiaY({ section, modularRatio }: any) {
  ...
  const transformedBars = bars.map((bar: any) => ({
    area: modularRatio * bar.area,
    y: bar.y,
  }));
  const totalArea =
    concreteArea + transformedBars.reduce((sum: any, bar: any) => sum + bar.area, 0);
  const centroid =
    (concreteArea * concreteCentroid +
      transformedBars.reduce((sum: any, bar: any) => sum + bar.area * bar.y, 0)) /
    totalArea;
  const inertia =
    concrete.inertiaY +
    concreteArea * (concreteCentroid - centroid) ** 2 +
    transformedBars.reduce((sum: any, bar: any) => sum + bar.area * (bar.y - centroid) ** 2, 0);
...
function crackingMoments({ section, concreteMaterial, transformedGross }: any) {
  const concrete = section.concreteSection;
...
function selectCrackingMoment(moment: any, crackingThresholds: any) {
  return moment >= 0 ? crackingThresholds.positive : crackingThresholds.negative;
}

function summarizeCurvaturePoint(point: any, { includePointDetails = false }: any = {}) {
 ...
 */
function countBendingSupportRestraints(beamModel: any) {
...
export class CrackedSectionDeflectionAnalysis {
  ...
  constructor({ code = "NTC2018", metadata = {} }: CrackedSectionDeflectionAnalysisOptions = {}) {
    this.code = code;
  ...
  analyze({
    ...
    const includeShrinkage =
      serviceability.deflection?.includeShrinkage ?? serviceability.includeShrinkage ?? false;
    const freeShrinkageStrain =
      serviceability.deflection?.freeShrinkageStrain ?? serviceability.freeShrinkageStrain ?? null;
    const limitRatio =
    ...
    const axialForceCurveTolerance = serviceability.deflection?.axialForceCurveTolerance ?? 1000;
    ...
    if (!isFinitePositive(axialForceCurveTolerance)) {
      throw new Error("RC cracked deflection axialForceCurveTolerance must be positive.");
    }
    ...
    if (includeShrinkage && !Number.isFinite(freeShrinkageStrain)) {
      return new VerificationResult({
        ...
        assumptions,
        metadata: deflectionMetadata(
          { code: this.code, beamId, ...this.metadata },
          includeShrinkage,
        ),
      });
    }

    const resultResolver = createUnitResolver(analysisResult.units, DEFAULT_RC_SECTION_UNITS);
    ...
    const getServiceContext = (effectiveModularRatio: any) => {
    ...
    const isHyperstatic =
      ...
      (hyperstatic === true ||
        (hyperstatic == null && countBendingSupportRestraints(normalizedBeamModel) > 2));
    ...
    const analysisCombinations: Record<string, any> = analysisResult.combinations ?? {};

    if (hyperstaticIteration) {
      for (const candidate of Object.values(analysisCombinations)) {
        if (
          String(candidate.context?.limitState ?? "").toUpperCase() !== "SLE" ||
          !candidate.factors ||
        ...
        const candidateCombinationType = candidate.context?.combinationType ?? null;
        const candidateCreepCoefficient = isQuasiPermanent(candidateCombinationType) ? phi : 0;
        ...
        for (const sample of candidate.internalForces?.samples ?? []) {
          ...
          const candidateMoment = Math.abs(resultResolver.moment(sample.m ?? 0) ?? 0);

          hyperstaticCurveEnvelopeByKey.set(
            candidateKey,
            Math.max(hyperstaticCurveEnvelopeByKey.get(candidateKey) ?? 0, candidateMoment),
          );
        }
      }
    }

    for (const result of Object.values(analysisCombinations)) {
      ...
      const grossReinforcementFirstMoment = section
        ...
        .reduce((sum: any, bar: any) => sum + bar.area * (bar.y - gross.centroid), 0);
      ...
      if (
        ...
        const initialMaxMoment = maxAbsSampleAction(initialSamples, "m", resultResolver);
        const governingInitialSample = initialSamples.reduce(
          (selected: any, sample: any) =>
            selected == null || Math.abs(sample.m ?? 0) > Math.abs(selected.m ?? 0)
              ? sample
              : selected,
          null,
        );
        const curveForAxialForce = (axialForce: any, requiredMoment = 0) => {
          const representativeAxialForce = quantizeAxialForce(axialForce, axialForceCurveTolerance);
          const curveKey = [
          ...
          if (!curve) {
            ...
            curve = new SectionMomentCurvatureCurve({
              ...
              solver: analysisOptions.solver,
              mcr: mcr as any,
              mcrPositive: crackingThresholds.positive as any,
              mcrNegative: crackingThresholds.negative as any,
        ...
        const representativeAxialForce = resultResolver.force(governingInitialSample?.n ?? 0);
        iteratedCurve = curveForAxialForce(representativeAxialForce, initialMaxMoment);
        iteratedCurveResolver = ({ axialForce, moment }: any) =>
        ...
        performance.femSolveCount += iteratedResult.performance?.femSolveCount ?? 0;
        performance.femSolveElapsedMs += iteratedResult.performance?.femSolveElapsedMs ?? 0;
        performance.hyperstaticIterationElapsedMs += iteratedResult.performance?.elapsedMs ?? 0;
      ...
      const analysisPoints = selectAnalysisSamples(rawPoints, analysisOptions.sampling);
      ...
      const curvaturePoints = analysisPoints.map(({ x, sample }) => {
        const mEd = (resultResolver.moment(sample.m ?? 0) ?? 0) as number;
        const nEd = (resultResolver.force(sample.n ?? 0) ?? 0) as number;
        const absM = Math.abs(mEd);
        const selectedMcr = selectCrackingMoment(mEd, crackingThresholds);
        const uncrackedCurvature = isFinitePositive(effectiveConcreteModulus * gross.inertia)
        ...
        if (iteratedCurve) {
          const pointCurve =
            iteratedCurveResolver?.({ axialForce: nEd, moment: mEd }) ?? iteratedCurve;
          ...
          const crackedShrinkageCurvature =
            includeShrinkage && creepCoefficient > 0 && crackedSection
              ? calculateEn1992ShrinkageCurvature({
                  ...
                  reinforcementElasticModulus: es,
                  effectiveConcreteModulus,
                  reinforcementFirstMoment: crackedSection.reinforcementFirstMoment,
                  sectionSecondMoment: crackedSection.inertia,
                }).curvature
              : uncrackedShrinkageCurvature;
          const shrinkageCurvature =
            (1 - zeta) * uncrackedShrinkageCurvature + zeta * crackedShrinkageCurvature;
          ...
          return {
            ...
            zeta,
            uncrackedCurvature: curveState.kappaUncracked ?? uncrackedCurvature,
            crackedCurvature,
        ...
        if (isFinitePositive(absM) && (!isFinitePositive(selectedMcr) || absM > selectedMcr)) {
          const solveCacheKey = [
            numericCacheKey(effectiveModularRatio),
            numericCacheKey(nEd),
            numericCacheKey(mEd),
        ...
        const loadCurvature = zeta * crackedCurvature + (1 - zeta) * uncrackedCurvature;
        const crackedShrinkageCurvature =
          includeShrinkage && creepCoefficient > 0 && crackedSection
            ? calculateEn1992ShrinkageCurvature({
                freeShrinkageStrain,
      ...
      const integrated = integrateCurvature(
        curvaturePoints,
        convertSupportStations(iteratedResult?.supports ?? result.supports ?? [], resultResolver),
        {
      ...
      const limit = isFinitePositive(limitRatio) ? (span as number) / limitRatio : null;
      ...
      const crackedPointCount = integrated.filter((point) => point.cracked).length;
      const maxZeta = integrated.reduce((max, point) => Math.max(max, point.zeta ?? 0), 0);
      const hyperstaticSummary = iteratedResult
        ? {
            ...
            relaxationFactor:
              iteratedResult.relaxationFactor ?? hyperstaticIteration?.relaxationFactor,
            ...
            adaptiveAdjustmentCount: iteratedResult.adaptiveAdjustmentCount ?? 0,
            ...
            curveLookupCount,
            femSolveCount: iteratedResult.performance?.femSolveCount ?? 0,
            femSolveElapsedMs: iteratedResult.performance?.femSolveElapsedMs ?? 0,
            iterationElapsedMs: iteratedResult.performance?.elapsedMs ?? 0,
          }
        : {
            active: false,
            converged: null,
      ...
      combinationOutputs.push({
        ...
        grossFlexuralRigidity: round(effectiveConcreteModulus * gross.inertia),
        ...
        points: outputPoints.map((point) => summarizeCurvaturePoint(point, analysisOptions.output)),
      });

      performance.returnedPointCount += (
        combinationOutputs[combinationOutputs.length - 1] as any
      ).returnedPointCount;
    }
    ...
    const governing: any = governingCheck(
      checks.map((check) => ({
        ...check,
        utilizationRatio: check.utilizationRatio ?? null,
      })),
    );
      ...
      outputs: {
        ...
        simplifiedSlenderness: slenderness
          ? {
              ...
              ok: slenderness.ok,
              metadata: { ...(slenderness.metadata as Record<string, unknown>) },
            }
          : null,
