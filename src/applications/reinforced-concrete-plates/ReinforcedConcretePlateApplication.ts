...
import type { ReinforcedConcretePlateApplicationInput } from "./types.js";
...
export class ReinforcedConcretePlateApplication extends StructuralApplication {
  constructor() {
    super({
      ...
      name: "RC Plates",
      description:
        "Local verification of flat reinforced-concrete plates through rotated Wood-Armer equivalent strips.",
  ...
  override run(input: ReinforcedConcretePlateApplicationInput = {}): VerificationResult {
    if (!input.model) {
      throw new Error("ReinforcedConcretePlateApplication requires a model.");
    }

    const model =
      input.model instanceof ReinforcedConcretePlateModel
        ? input.model
