...
import {
  ...
  type ReinforcedConcretePlateModel,
} from "./ReinforcedConcretePlateModel.js";
import type {
  RcPlateState,
  ReinforcedConcretePlateVerificationOptions,
  TransformedRcPlateState,
} from "./types.js";

const SLE_COMBINATIONS: ReadonlySet<string> = new Set([
  "SLE_RARE",
  "SLE_CHARACTERISTIC",
  "SLE_CHAR",
  "SLE_FREQUENT",
...
interface PlateVerificationOutputs extends Record<string, unknown> {
  geometry: Record<string, unknown>;
  reinforcement: unknown;
  sourceActions: Array<Record<string, unknown>>;
  reinforcementAxisActions: Array<Record<string, unknown>>;
  woodArmerMoments: Array<Record<string, unknown>>;
  bendingChecks: unknown[];
  shearChecks: unknown[];
  serviceStressChecks: VerificationCheck[];
  crackingChecks: VerificationCheck[];
  slendernessChecks: unknown[];
  serviceabilityStripResults?: unknown[];
  governingCheck: VerificationCheck | null;
}
...
function transformState(
  ...
  const torsionRatio = directMoment > 0 ? Math.abs(moments.mxy) / directMoment : null;
  ...
  return {
    ...
    torsionRatio,
    torsionDiagnostic:
      directMoment === 0 && Math.abs(moments.mxy) > 0
        ? "pure-torsion"
        : "ratio-to-maximum-direct-moment",
  };
}

function incompatibleCombination(
  model: ReinforcedConcretePlateModel,
  transformedStates: readonly TransformedRcPlateState[],
): TransformedRcPlateState | null {
  if (model.analysis.type === RC_PLATE_ANALYSIS_TYPES.SLS_STRESS_CRACKING) {
    return (
      transformedStates.find(
        (state) => state.combinationType == null || !SLE_COMBINATIONS.has(state.combinationType),
      ) ?? null
    );
  }
...
function governingCheck(checks: readonly VerificationCheck[]): VerificationCheck | null {
  return checks.reduce<VerificationCheck | null>((governing, check) => {
    if (!Number.isFinite(check.utilizationRatio)) {
      return governing;
    }

    return governing == null ||
      (check.utilizationRatio as number) > (governing.utilizationRatio as number)
      ? check
      : governing;
  }, null);
}
...
function baseOutputs(
  ...
  return {
    ...
    reinforcementAxisActions: transformedStates.map((state) => ({
      ...
      sourceShearResultant: Math.hypot(state.sourceActions.qx, state.sourceActions.qy),
...
export class ReinforcedConcretePlateVerification {
  ...
  verify(model: ReinforcedConcretePlateModel): VerificationResult {
    if (!Array.isArray(model?.analysis?.states)) {
      throw new Error(
        "ReinforcedConcretePlateVerification requires a ReinforcedConcretePlateModel.",
      );
    }
    ...
    if (incompatible) {
      ...
      return new VerificationResult({
        ...
        summary:
          "RC plate verification was not run because the combination type is missing or incompatible.",
        checks: [],
        outputs,
        warnings,
        assumptions,
    ...
    const atomicChecks: VerificationCheck[] = [];
    ...
    if (model.analysis.type === RC_PLATE_ANALYSIS_TYPES.ULS_BENDING_SHEAR) {
      ...
      for (const transformedState of transformedStates) {
        const serviceability = verifyPlateServiceability({
          model,
          transformedState,
        });
        ...
        outputs.crackingChecks.push(...serviceability.flatMap((item) => item.crackingChecks));
        ...
        const highTorsion =
          transformedState.torsionDiagnostic === "pure-torsion" ||
          (transformedState.torsionRatio != null && transformedState.torsionRatio >= 1);
      ...
      for (const transformedState of transformedStates) {
        ...
        atomicChecks.push(
          ...slenderness.flatMap((item) => item.faceChecks.map((faceCheck) => faceCheck.check)),
        );
    ...
    const status =
      childStatuses.every((statusValue) => statusValue === RESULT_STATUS.OK) &&
    ...
    return new VerificationResult({
      ...
      status,
      summary:
        model.analysis.type === RC_PLATE_ANALYSIS_TYPES.SLS_SIMPLIFIED_DEFLECTION
      ...
      utilizationRatio:
        typeof governing?.utilizationRatio === "number" ? governing.utilizationRatio : null,
  ...
  resultMetadata(model: ReinforcedConcretePlateModel): Record<string, unknown> {
    const normativeReferencesByAnalysis: Record<string, NormativeReference[]> = {
      [RC_PLATE_ANALYSIS_TYPES.ULS_BENDING_SHEAR]: [
        NTC2018_RC_CHAPTER_4_REFERENCES.flexureAndAxialForce,
        NTC2018_RC_CHAPTER_4_REFERENCES.shearWithoutTransverseReinforcement,
      ],
      [RC_PLATE_ANALYSIS_TYPES.SLS_STRESS_CRACKING]: [
        NTC2018_RC_CHAPTER_4_REFERENCES.concreteStressCharacteristic,
        NTC2018_RC_CHAPTER_4_REFERENCES.concreteStressQuasiPermanent,
        NTC2018_RC_CHAPTER_4_REFERENCES.reinforcementStress,
        NTC2018_RC_CHAPTER_4_REFERENCES.crackWidth,
        CIRC2019_RC_REFERENCES.indirectCrackControlDiameter,
        CIRC2019_RC_REFERENCES.indirectCrackControlSpacing,
      ],
      [RC_PLATE_ANALYSIS_TYPES.SLS_SIMPLIFIED_DEFLECTION]: [
        NTC2018_RC_CHAPTER_4_REFERENCES.deflection,
        CIRC2019_RC_REFERENCES.simplifiedDeflectionSlenderness,
      ],
    };
    const normativeReferences = normativeReferencesByAnalysis[model.analysis.type] ?? [];
