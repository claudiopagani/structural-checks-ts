// Mechanical TypeScript migration from strutture-js 6f33baead8b88166c4b2cf94af41763412e3c751; source path: ../strutture-js/src/applications/reinforced-concrete-strut-and-tie/ReinforcedConcreteStrutAndTieModel.js.
...
const STRENGTH_MODELS = new Set(["uncracked-uniaxial", "transverse-tension"]);
const NODE_TYPES = new Set(["ccc", "cct", "ctt"]);
const FORCE_REFERENCE_KINDS = new Set(["member", "load", "reaction", "explicit"]);
...
function normalizedDirection(value: any, label: string): any {
...
function normalizeFactors(factors: any = {}): any {
...
export class ReinforcedConcreteStrutAndTieModel {
  ...
  constructor({
    ...
    if (
      !Number.isFinite(concreteMaterial.elasticModulus) ||
    ...
    const normalizedMembers = (members as any[]).map((member: any) => {
      ...
      const area = positive(resolver.area(Number(member.area)), `member ${member.id} area`);
      const material = type === "strut" ? concreteMaterial : reinforcementMaterial;
      const elasticModulus =
        member.analysisElasticModulus == null
      ...
      return {
        ...
        area,
        axialRigidity:
          area * positive(elasticModulus, `member ${member.id} analysis elastic modulus`),
        strengthModel: type === "strut" ? member.strengthModel : null,
        metadata: { ...(member.metadata ?? {}) },
      };
    });
    const normalizedNodes = (nodes as any[]).map((node: any) => ({
      id: node.id,
      x: resolver.length(Number(node.x)),
      y: resolver.length(Number(node.y)),
      metadata: { ...(node.metadata ?? {}) },
    }));
    const normalizedLoads = (loads as any[]).map((load: any) => ({
      id: load.id,
      nodeId: load.nodeId,
    ...
    const normalizedSupports = (supports as any[]).map((support: any) => ({ ...support }));
    ...
    const nodeIds = new Set(this.domainModel.nodes.map((node: any) => node.id));
    const memberById = new Map<string, any>(
      this.domainModel.members.map((member: any) => [member.id, member]),
    );
    const loadById = new Map<string, any>(
      this.domainModel.loads.map((load: any) => [load.id, load]),
    );
    ...
    this.nodalZones = (nodalZones as any[]).map((zone: any) => {
      ...
      if (kind === "reaction" && (reference.nodeId ?? zone.nodeId) !== zone.nodeId) {
        throw new Error(`Nodal zone ${zone.id} reaction must belong to node ${zone.nodeId}.`);
      }
      ...
      return {
        ...
        nodeId: zone.nodeId,
        type: zone.type,
        area: positive(resolver.area(Number(zone.area)), `nodal zone ${zone.id} area`),
        forceReference: {
          kind,
    ...
    this.id = id;
    this.members = normalizedMembers.map((member: any) => ({ ...member }));
