// Mechanical TypeScript migration from strutture-js 6f33baead8b88166c4b2cf94af41763412e3c751; source path: ../strutture-js/src/applications/reinforced-concrete-strut-and-tie/ReinforcedConcreteStrutAndTieVerification.js.
...
function resultMetadata({ code, modelId, metadata = {}, ...details }: any): any {
  return withNormativeReferences(
    ...
    [NTC2018_RC_CHAPTER_4_REFERENCES.strutAndTie, EN1992_RC_EXTERNAL_REFERENCES.strutAndTie],
  );
}

function incompatibleMechanismCheck(member: any): any {
  return {
...
function project(vector: any, normal: any): any {
  return Math.abs(vector.fx * normal.x + vector.fy * normal.y);
}

function resolveZoneForce(zone: any, analysis: any): any {
  ...
  if (reference.kind === "member") {
    const member = analysis.members.find((value: any) => value.id === reference.id);
    return Math.abs(member.force);
  }

  if (reference.kind === "load") {
    const load = analysis.loads.find((value: any) => value.id === reference.id);
    return project(load, reference.normal);
  }

  return project(analysis.reactions[reference.nodeId] ?? { fx: 0, fy: 0 }, reference.normal);
}

export class ReinforcedConcreteStrutAndTieVerification {
  [key: string]: any;

  constructor({ code = SUPPORTED_CODE, metadata = {} }: any = {}) {
    this.code = code;
    this.metadata = { ...metadata };
  }

  verify(model: any): any {
    ...
    const sourceMemberById = new Map<string, any>(
      model.members.map((member: any) => [member.id, member]),
    );
    const memberChecks = [];
    const memberCapacities: Record<string, any> = {};
    const incompatibleMembers = [];

    for (const member of analysis.members as any[]) {
    ...
    const nodalZoneResults = model.nodalZones.map((zone: any) => {
    ...
    const checks = [...memberChecks, ...nodalZoneResults.map((zone: any) => zone.check)];
    ...
    return new VerificationResult({
      ...
      outputs: {
        ...
        memberCapacities,
        nodalZones: nodalZoneResults.map((value: any) => {
          const zone = { ...value };
          delete zone.check;
          return zone;
        }),
