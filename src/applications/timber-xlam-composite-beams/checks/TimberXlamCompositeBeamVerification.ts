// Mechanical TypeScript migration from strutture-js 6f33baead8b88166c4b2cf94af41763412e3c751; source path: src/applications/timber-xlam-composite-beams/checks/TimberXlamCompositeBeamVerification.js.
/* eslint-disable @typescript-eslint/ban-ts-comment, @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-argument, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-return */
// @ts-nocheck
...
function femDemands(analysisResult) {
  ...
  const finalEntries = sleEntries.filter(
    (entry) =>
      entry.context?.serviceCombination === "final" || entry.context?.deformationState === "final",
  );
  ...
  return {
    ...
    deflectionShort: shortDeflection ? Math.abs(resolver.length(shortDeflection.value)) : null,
    deflectionLong: longDeflection ? Math.abs(resolver.length(longDeflection.value)) : null,
    source: "fem-diagrams",
  };
}
...
export class TimberXlamCompositeBeamVerification {
  ...
  verifySectionActions({ nEd = 0, vEd, mEd, principalActions, context = {} }: any = {}) {
    ...
    const timberTimberResistance =
      connector.timberTimberCharacteristicResistance(xlamThicknessActive);
    const connectorForce = ((gamma1Uls * e1 * a1 * a1Uls * seq) / ejEffUls) * shearEd;
    ...
    const timberShearResistance = (kmod / gammaTimber) * timberShearStrength(timberMaterial);
