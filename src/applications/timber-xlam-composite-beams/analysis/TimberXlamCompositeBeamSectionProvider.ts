// Mechanical TypeScript migration from strutture-js 6f33baead8b88166c4b2cf94af41763412e3c751; source path: src/applications/timber-xlam-composite-beams/analysis/TimberXlamCompositeBeamSectionProvider.js.
/* eslint-disable @typescript-eslint/ban-ts-comment, @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-return */
// @ts-nocheck
...
export class TimberXlamCompositeBeamSectionProvider {
  ...
  calculateGammaProperties(context = {}) {
    const { span, xlamSection, timberSection, xlamMaterial, timberMaterial, connector } =
      this.model;
    ...
    const stiffnessReduction = finalStiffness && Number.isFinite(kdef) ? 1 + kdef : 1;
    ...
    const gamma1 = 1 / (1 + (Math.PI ** 2 * e1 * a1 * spacing) / (connectorStiffness * span ** 2));
    const gamma2 = 1 / (1 + (Math.PI ** 2 * e2 * a2 * spacing) / (connectorStiffness * span ** 2));
    const a2Lever = (gamma1 * e1 * a1 * a) / (gamma1 * e1 * a1 + e2 * a2);
    const a1Lever = a - a2Lever;
    const flexuralRigidity =
      e1 * j1 + e2 * j2 + gamma1 * e1 * a1 * a1Lever ** 2 + gamma2 * e2 * a2 * a2Lever ** 2;
