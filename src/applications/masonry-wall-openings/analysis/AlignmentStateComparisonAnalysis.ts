...
interface ComparisonOptions extends JsonRecord {
  stiffnessToleranceRatio?: number;
  stiffnessVariationTolerance?: number;
  stiffnessTolerance?: number;
  stateOfFact?: JsonRecord;
  stateOfFactOptions?: JsonRecord;
  design?: JsonRecord;
  designOptions?: JsonRecord;
}

interface SplitComparisonOptions {
  stiffnessToleranceRatio: number;
  stateOfFactOptions: JsonRecord;
  designOptions: JsonRecord;
}

interface ComparisonCheck extends JsonRecord {
  id: string;
  description: string;
...
export interface AlignmentStateComparisonAnalysisInput {
  alignment?: MasonryWallOpeningsModel | null;
  options?: ComparisonOptions;
  sanitizedOpenings?: readonly SanitizedAlignmentOpening[] | null;
}

export interface AlignmentStateComparisonAnalysisOutputs extends JsonRecord {
  criteria: JsonRecord & {
    stiffnessToleranceRatio: number;
    stiffnessTolerancePercent: number;
    strengthMustNotDecrease: boolean;
    deformabilityMustNotDecrease: boolean;
  };
}

export type AlignmentStateComparisonAnalysisResult =
  CalculationResult<AlignmentStateComparisonAnalysisOutputs>;
...
function resolveStiffnessToleranceRatio(options: JsonRecord = {}): number {
  const candidate =
...
function splitComparisonOptions(options: ComparisonOptions = {}): SplitComparisonOptions {
  const {
...
function resolveRatio(referenceValue: unknown, candidateValue: unknown): number | null {
  if (
    !isFiniteNumber(referenceValue) ||
...
function resolveVariationRatio(referenceValue: unknown, candidateValue: unknown): number | null {
  const ratio = resolveRatio(referenceValue, candidateValue);

  return isFiniteNumber(ratio) ? ratio - 1 : null;
}

function numericDifference(left: unknown, right: unknown): number {
  return isFiniteNumber(left) && isFiniteNumber(right) ? left - right : Number.NaN;
}
...
function buildVariationBandCheck({
  ...
  return {
    ...
    ratio: round(ratio),
    delta: round(numericDifference(candidateValue, referenceValue)),
    variationRatio: round(variationRatio),
    variationPercent: round(isFiniteNumber(variationRatio) ? variationRatio * 100 : null),
    ok,
  };
}
...
function summarizeSeismicResult(result: AlignmentSeismicAggregatedAnalysisResult): JsonRecord {
...
function signedPercentText(value: unknown): string | null {
...
function buildCheckReading(check: ComparisonCheck | undefined): string | null {
  if (!check) {
    return null;
  }

  if (check.id === "stiffness-variation") {
    if (!isFiniteNumber(check.variationPercent)) {
...
function buildComparisonReading({
  ...
  const messages = checks.map((check) => buildCheckReading(check)).filter(Boolean);
...
export class AlignmentStateComparisonAnalysis {
  analyze({
  ...
  }: AlignmentStateComparisonAnalysisInput = {}): AlignmentStateComparisonAnalysisResult {
    if (!alignment) {
      throw new Error("AlignmentStateComparisonAnalysis requires an alignment model.");
    }
    ...
    const { stiffnessToleranceRatio, stateOfFactOptions, designOptions } =
    ...
    for (const check of checks.filter((candidate) => !candidate.ok)) {
      if (check.id === "stiffness-variation") {
        const absoluteVariationPercent = isFiniteNumber(check.variationPercent)
          ? Math.abs(check.variationPercent)
          : Number.NaN;
        warnings.push(
          `The project-stage stiffness changes by ${round(absoluteVariationPercent)}% relative to the state-of-fact result, beyond the configured +/-${round(stiffnessToleranceRatio * 100)}% comparison band.`,
        );
        continue;
      }
      ...
      if (check.id === "strength-non-decreasing") {
        warnings.push(
          `The project-stage equivalent strength Vy (${formatValue(round(check.designValue))}) is lower than the state-of-fact value (${formatValue(round(check.stateOfFactValue))}).`,
        );
        continue;
      }
      ...
      if (check.id === "deformability-non-decreasing") {
        warnings.push(
          `The project-stage ultimate displacement du (${formatValue(round(check.designValue))}) is lower than the state-of-fact value (${formatValue(round(check.stateOfFactValue))}).`,
        );
      }
    }
    ...
    return new CalculationResult({
      ...
      outputs: {
        ...
        comparison: {
          topRotation:
            designResult.outputs?.topRotation ?? stateOfFactResult.outputs?.topRotation ?? null,
          includeSpandrels:
          ...
          du: checks.find((check) => check.id === "deformability-non-decreasing"),
          overall: {
            ok: allChecksOk,
            status,
            failedCheckIds: checks.filter((check) => !check.ok).map((check) => check.id),
          },
        },
        reading,
      },
