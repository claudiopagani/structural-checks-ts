// Mechanical TypeScript migration from strutture-js 6f33baead8b88166c4b2cf94af41763412e3c751; source path: src/applications/masonry-wall-openings/MasonryWallOpeningsApplication.js.
...
import {
  ...
  type AlignmentEquivalentFramePushoverAnalysisOptions,
} from "./analysis/AlignmentEquivalentFramePushoverAnalysis.js";
import {
  AlignmentSeismicAggregatedAnalysis,
  type AlignmentSeismicAggregatedAnalysisInput,
} from "./analysis/AlignmentSeismicAggregatedAnalysis.js";
import {
  AlignmentStateComparisonAnalysis,
  type AlignmentStateComparisonAnalysisInput,
} from "./analysis/AlignmentStateComparisonAnalysis.js";
import {
  AlignmentStaticAnalysis,
  type AlignmentStaticAnalysisInput,
} from "./analysis/AlignmentStaticAnalysis.js";
...
import {
  ...
  type AlignmentStateComparisonReportBuilderReport,
...
export interface MasonryWallOpeningsComparisonReportBuilder {
  build(
    input: AlignmentStateComparisonReportBuilderBuildInput,
  ): AlignmentStateComparisonReportBuilderReport;
}

export interface MasonryWallOpeningsApplicationOptions {
  comparisonReportBuilder?: MasonryWallOpeningsComparisonReportBuilder;
}

export interface MasonryWallOpeningsApplicationInput extends MasonryWallOpeningsModelInput {
  mode?: string;
  analysisType?: string;
  stage?: string;
  options?: JsonRecord;
  staticOptions?: AlignmentStaticAnalysisInput["options"];
  seismicOptions?: AlignmentSeismicAggregatedAnalysisInput["options"];
  frameOptions?: AlignmentEquivalentFramePushoverAnalysisOptions;
  compareOptions?: AlignmentStateComparisonAnalysisInput["options"];
  metadata?: JsonRecord;
  model?: MasonryWallOpeningsModel | MasonryWallOpeningsModelInput | null;
}

function resolveMode(input: MasonryWallOpeningsApplicationInput): string {
...
export class MasonryWallOpeningsApplication extends StructuralApplication {
  private comparisonReportBuilder: MasonryWallOpeningsComparisonReportBuilder;
  ...
  override run(input: MasonryWallOpeningsApplicationInput = { id: "" }): CalculationResult {
    const model =
      input.model instanceof MasonryWallOpeningsModel
        ? input.model
        : new MasonryWallOpeningsModel(input.model ?? input);
    const mode = resolveMode(input);
    const sanitization: SanitizeAlignmentOpeningsResult = sanitizeAlignmentOpenings({
      alignment: model,
    });
    const extraction =
    ...
    if (mode === "extract-equivalent-frame-members" || mode === "extract-members") {
      const extractedMembers: EquivalentFrameMembersResult =
        extraction ??
        extractEquivalentFrameMembers({
          alignment: model,
          sanitizedOpenings: sanitization.openings,
      ...
      return new CalculationResult({
        ...
        outputs: {
          modelId: model.id,
          sanitizedOpenings: sanitization.openings,
          piers: extractedMembers.piers.map((pier) => pier.toJSON()),
          spandrels: extractedMembers.spandrels.map((spandrel) => spandrel.toJSON()),
          discardedOpeningIds: sanitization.discardedOpeningIds,
        },
        warnings: [...sanitization.warnings, ...extractedMembers.warnings],
        assumptions: extractedMembers.assumptions,
        metadata: {
          domain: this.domain,
          mode,
          ...sanitization.metadata,
          ...extractedMembers.metadata,
        },
      });
    }

    if (mode === "static-design" || mode === "static-state-of-fact" || mode === "static") {
    ...
    if (mode === "equivalent-frame-linear" || mode === "equivalent-frame") {
      ...
      const frameMetadata = isRecord(frame.snapshot.metadata) ? frame.snapshot.metadata : {};

      return new CalculationResult({
        applicationId: this.id,
        status: RESULT_STATUS.OK,
        summary:
          Number(frameMetadata.ringFrameCount) > 0
            ...
            : frameMetadata.frameType === "pier-spandrel"
