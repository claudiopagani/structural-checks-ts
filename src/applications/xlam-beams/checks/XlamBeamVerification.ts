// Mechanical TypeScript migration from strutture-js 6f33baead8b88166c4b2cf94af41763412e3c751; source path: src/applications/xlam-beams/checks/XlamBeamVerification.js.
/* eslint-disable @typescript-eslint/ban-ts-comment, @typescript-eslint/no-unsafe-argument, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-return, @typescript-eslint/no-unused-vars */
// @ts-nocheck
...
function xlamRollingShearDemand({ section, material, vEd, bendingStiffness }) {
  let tau0Max = 0;
  let tau90Max = 0;
    ...
    for (const upperLayer of section.layers) {
      ...
      const localModulus = upperLayer.active
        ...
        : materialValue(
            material,
            ["e90Mean"],
            materialValue(material, ["e0Mean", "elasticModulus"]) / 30,
          );
  ...
  return {
    verifySectionActions({ nEd, vEd, mEd, principalActions, context }) {
      ...
      const rollingShearStrength =
        materialValue(material, ["rollingShearStrength", "fvK"]) ?? material.metadata?.fvK;
      const bendingStiffness = section.calculateBendingStiffness(material, {
        includeCrossLayerBending,
      });
      const edgeDistance = Math.max(section.centroidY, section.height - section.centroidY);
      const sigmaEdge = (Math.abs(convertedMEd) * e0 * edgeDistance) / bendingStiffness;
      ...
      if (Math.abs(convertedNEd) > 1e-9) {
        warnings.push("Axial force is not included in the current XLAM strip stress checks.");
      }
  ...
  verify({
    ...
    if (!section || !material || !analysisResult) {
      return new VerificationResult({
        ...
        summary:
          "XLAM beam verification requires a section, material and FEM beam analysis result.",
        warnings: ["XLAM strip verification was not run because required inputs are missing."],
        metadata: {
          beamId,
          ...this.metadata,
        },
    ...
    const deflectionCheck =
      governingDeflection && Number.isFinite(span)
        ...
        : null;
    const checks = [...actionVerification.checks, ...(deflectionCheck ? [deflectionCheck] : [])];
    ...
    return new VerificationResult({
      ...
      summary:
        "XLAM strip beam bending, rolling shear and deflection verification from FEM results.",
      utilizationRatio: governing?.utilizationRatio ?? null,
      demand: governing?.demand ?? null,
      capacity: governing?.capacity ?? null,
