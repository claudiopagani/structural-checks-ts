// Mechanical TypeScript migration from strutture-js 6f33baead8b88166c4b2cf94af41763412e3c751; source path: src/applications/timber-concrete-composite-beams/checks/TimberConcreteCompositeBeamVerification.js.
...
function demandResolver(context = {}) {
  return createUnitResolver(context.units ?? { force: "N", length: "mm" }, {
    force: "N",
    length: "mm",
  });
}
...
export class TimberConcreteCompositeBeamVerification {
  ...
  verifySectionActions({ nEd = 0, vEd, mEd, principalActions, context = {} }: any = {}) {
    ...
    const reinforcementArea = reinforcement.distributedArea(slabWidth, reinforcementSpacing);
    ...
    const gammaUls =
      1 /
      (1 +
        (Math.PI ** 2 * ew * (idealInertia - disconnectedInertia) * connectorSpacing) /
    ...
    const timberStressBottom = slipForce / timberArea - timberMoment / timberSectionModulus;
    const timberStressTop = slipForce / timberArea + timberMoment / timberSectionModulus;
    const neutralAxisDepth = (slipForce + reinforcementArea * fyd) / (0.8 * slabWidth * fcd);
    ...
    const checks = [
      ...
      },
      {
        ...
        ...evaluateCheck(Math.abs(connectorForce), (kmod * connectorFvrk) / gammaConnector),
      },
    ].map((check) => ({
      ...check,
  ...
  verify(input) {
    ...
    const verificationStations =
      input?.verificationStations ?? model?.verificationStations ?? this.verificationStations;
    ...
    const inertiaEffSle = disconnectedInertia + gammaSle * (idealInertia - disconnectedInertia);
    ...
    const deflectionEffUls = (5 * qUls * span ** 4) / (384 * ew * inertiaEffUls);
    const deflectionIdealUls = (5 * qUls * span ** 4) / (384 * ew * idealInertia);
    ...
    const deflectionSle =
      demands?.deflectionSle ?? (5 * qSleRare * span ** 4) / (384 * ewInf * inertiaEffSle);

    const timberBottomCheck = evaluateCheck(Math.abs(timberStressBottom), fmD);
    ...
    const actionChecks = sectionActionVerification
      ...
      : checks.slice(0, 4);
    const combinedChecks = [...actionChecks, checks.find((check) => check.id === "deflection")];
    ...
    return new VerificationResult({
      applicationId: "timber-concrete-composite-beams",
      status: combinedChecks.every((check) => check.ok)
        ? RESULT_STATUS.OK
