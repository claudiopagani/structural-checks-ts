...
export type PileTransferLawKind = "p-y";
export const PILE_TRANSFER_LAW_KINDS = Object.freeze([
  "p-y",
]) satisfies readonly PileTransferLawKind[];

export type PileTransferCurveModel = "symmetric-piecewise-linear";
export const PILE_TRANSFER_CURVE_MODELS = Object.freeze([
  "symmetric-piecewise-linear",
]) satisfies readonly PileTransferCurveModel[];

export type PileTransferExtrapolationModel = "constant" | "linear";
export const PILE_TRANSFER_EXTRAPOLATION_MODELS = Object.freeze([
  "constant",
  "linear",
]) satisfies readonly PileTransferExtrapolationModel[];

export interface PileTransferPointInput {
  displacement?: number;
  resistancePerLength?: number;
}

export interface PileTransferPoint {
  displacement: number;
  resistancePerLength: number;
}

export interface PileTransferLawOptions {
  id?: string;
  name?: string | null;
  kind?: string;
  curveModel?: string;
  points?: PileTransferPointInput[];
...
export interface PileTransferEvaluation {
  displacement: number;
...
export interface PileTransferLawJson {
  schemaVersion: string;
  id: string;
  name: string;
  kind: PileTransferLawKind;
  curveModel: PileTransferCurveModel;
  points: PileTransferPoint[];
  extrapolation: PileTransferExtrapolationModel;
...
function isPileTransferLawKind(value: unknown): value is PileTransferLawKind {
  return value === "p-y";
}

function isPileTransferCurveModel(value: unknown): value is PileTransferCurveModel {
  return value === "symmetric-piecewise-linear";
}

function isPileTransferExtrapolationModel(value: unknown): value is PileTransferExtrapolationModel {
  return value === "constant" || value === "linear";
}
...
function normalizePoints(
  points: PileTransferPointInput[] | null | undefined,
  ...
  const normalized = points.map((point, index) => ({
    displacement: resolver.length(finite(point.displacement, `points[${index}].displacement`)),
    resistancePerLength: resolver.lineLoad(
      finite(point.resistancePerLength, `points[${index}].resistancePerLength`),
    ),
  }));
  ...
  for (let index = 1; index < normalized.length; index += 1) {
    ...
    if (current.displacement <= previous.displacement + tolerance) {
      throw new Error("PileTransferLaw displacements must be positive and strictly increasing.");
    }
    if (current.resistancePerLength < previous.resistancePerLength - tolerance) {
      throw new Error("PileTransferLaw static-monotonic resistance must be non-decreasing.");
    }
...
interface PileTransferSegmentResponse {
  resistancePerLength: number;
  tangentModulus: number;
  segmentIndex: number;
  extrapolated: boolean;
...
function segmentResponse(
  ...
  return {
    resistancePerLength:
      left.resistancePerLength + tangentModulus * (magnitude - left.displacement),
    tangentModulus,
...
export class PileTransferLaw {
  ...
  constructor({
    ...
    if (!isPileTransferLawKind(kind)) {
      throw new Error(`Unsupported pile transfer-law kind: ${stringValue(kind)}.`);
    }
    if (!isPileTransferCurveModel(curveModel)) {
      throw new Error(`Unsupported pile transfer curve model: ${stringValue(curveModel)}.`);
    }
    if (!isPileTransferExtrapolationModel(extrapolation)) {
      throw new Error(
        `Unsupported pile transfer extrapolation model: ${stringValue(extrapolation)}.`,
      );
    }
    if (loading !== "static-monotonic") {
      throw new Error("PileTransferLaw currently supports only static-monotonic loading.");
    }
    ...
    this.metadata = {
      ...
      signConvention: {
        ...
        mobilizedResistance: "same sign as displacement; soil reaction on pile has opposite sign",
      },
    };
  }

  evaluate(displacement: number): PileTransferEvaluation {
    const signedDisplacement = finite(displacement, "displacement");
    const magnitude = Math.abs(signedDisplacement);
    const response = segmentResponse(this.points, magnitude, this.extrapolation);
    const sign = Math.sign(signedDisplacement);
    const mobilizedResistancePerLength = sign === 0 ? 0 : sign * response.resistancePerLength;
    return {
      ...
      secantModulus:
        magnitude > 0 ? response.resistancePerLength / magnitude : response.tangentModulus,
      segmentIndex: response.segmentIndex,
      extrapolated: response.extrapolated,
    };
  }

  toJSON(): PileTransferLawJson {
    return {
