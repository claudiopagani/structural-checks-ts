...
export interface GroundSectionPoint {
  x: number;
  z: number;
}

export interface GroundSectionPointInput {
  x?: number;
  z?: number;
}

export interface GroundSectionSurface {
  points: GroundSectionPoint[];
  metadata: Record<string, unknown>;
}

export interface GroundSectionSurfaceInput {
  points?: GroundSectionPointInput[];
  metadata?: Record<string, unknown>;
}

export interface GroundSectionZoneBounds {
  minimumX: number;
  maximumX: number;
  minimumZ: number;
  maximumZ: number;
...
export interface GroundSectionZone {
  id: string;
  materialId: string;
  polygon: GroundSectionPoint[];
  bounds: GroundSectionZoneBounds;
  metadata: Record<string, unknown>;
}

export interface GroundSectionZoneInput {
  id?: string;
  materialId?: string;
  polygon?: GroundSectionPointInput[];
  metadata?: Record<string, unknown>;
}

export interface GroundSectionZoneJson {
  id: string;
  materialId: string;
  polygon: GroundSectionPoint[];
  metadata: Record<string, unknown>;
...
export interface GroundSection2DOptions {
  id?: string;
  name?: string | null;
  surface?: GroundSectionSurfaceInput | null;
  zones?: GroundSectionZoneInput[];
  units?: UnitSystemInput | null;
  metadata?: Record<string, unknown>;
}

export interface GroundSection2DJson {
  ...
  surface: GroundSectionSurface;
  zones: GroundSectionZoneJson[];
  bounds: GroundSectionZoneBounds;
  units: typeof GEOTECHNICAL_INTERNAL_UNITS;
  metadata: Record<string, unknown>;
}

export interface GroundSectionPointQueryOptions {
...
function samePoint(
  left: GroundSectionPoint,
  right: GroundSectionPoint,
  tolerance: number,
): boolean {
  return Math.abs(left.x - right.x) <= tolerance && Math.abs(left.z - right.z) <= tolerance;
}

function normalizePoint(
  point: GroundSectionPointInput | null | undefined,
...
function signedPolygonArea(points: GroundSectionPoint[]): number {
  let doubledArea = 0;
...
function orientation(a: GroundSectionPoint, b: GroundSectionPoint, c: GroundSectionPoint): number {
  return (b.x - a.x) * (c.z - a.z) - (b.z - a.z) * (c.x - a.x);
}

function pointOnSegment(
  point: GroundSectionPoint,
...
function segmentsIntersect(
  ...
  const signs = values.map((value) => (Math.abs(value) <= tolerance ? 0 : Math.sign(value)));
...
function collinearOverlapWithSameInteriorSide(
  ...
  const leftMinimum = Math.min(useX ? startA.x : startA.z, useX ? endA.x : endA.z);
  const leftMaximum = Math.max(useX ? startA.x : startA.z, useX ? endA.x : endA.z);
  const rightMinimum = Math.min(useX ? startB.x : startB.z, useX ? endB.x : endB.z);
  const rightMaximum = Math.max(useX ? startB.x : startB.z, useX ? endB.x : endB.z);
...
function pointLocationInPolygon(
  point: GroundSectionPoint,
  polygon: GroundSectionPoint[],
  tolerance: number,
  ...
  for (let index = 0; index < polygon.length; index += 1) {
    ...
    const intersectionX =
      current.x + ((point.z - current.z) * (next.x - current.x)) / (next.z - current.z);
    if (intersectionX > point.x + tolerance) inside = !inside;
  }
  return inside ? "inside" : "outside";
}

function assertSimplePolygon(points: GroundSectionPoint[], label: string, tolerance: number): void {
...
function normalizePolygon(
  points: GroundSectionPointInput[] | null | undefined,
  resolver: ReturnType<typeof createUnitResolver>,
  label: string,
  tolerance: number,
): GroundSectionPoint[] {
...
function polygonBounds(polygon: GroundSectionPoint[]): GroundSectionZoneBounds {
  return {
...
function polygonsHaveInteriorOverlap(
  left: GroundSectionPoint[],
  right: GroundSectionPoint[],
  tolerance: number,
): boolean {
  ...
  if (left.some((point) => pointLocationInPolygon(point, right, tolerance) === "inside")) {
    return true;
  }
  if (right.some((point) => pointLocationInPolygon(point, left, tolerance) === "inside")) {
    return true;
  }

  const leftOnRight = left.every(
    (point) => pointLocationInPolygon(point, right, tolerance) === "boundary",
  );
  const rightOnLeft = right.every(
    (point) => pointLocationInPolygon(point, left, tolerance) === "boundary",
  );
  return leftOnRight && rightOnLeft;
}

function assertPolygonBelowSurface(
  polygon: GroundSectionPoint[],
  surfacePoints: GroundSectionPoint[],
  zoneId: string,
  tolerance: number,
): void {
  for (let index = 0; index < polygon.length; index += 1) {
    ...
    for (const surfacePoint of surfacePoints) {
      if (surfacePoint.x <= minimumX + tolerance || surfacePoint.x >= maximumX - tolerance) {
        continue;
      }
...
function normalizeSurface(
  surface: GroundSectionSurfaceInput | null | undefined,
  resolver: ReturnType<typeof createUnitResolver>,
  tolerance: number,
): GroundSectionSurface {
  const rawPoints = surface?.points;
...
function interpolatePolyline(points: GroundSectionPoint[], x: number, tolerance: number): number {
...
export class GroundSection2D {
  ...
  id: string;
  name: string;
  coordinateSystem: GroundSection2DJson["coordinateSystem"];
  surface: GroundSectionSurface;
  zones: GroundSectionZone[];
  bounds: GroundSectionZoneBounds;
  units: typeof GEOTECHNICAL_INTERNAL_UNITS;
  metadata: Record<string, unknown>;
  ...
  constructor({
    ...
    const normalizedZones = zones.map((zone, index): GroundSectionZone => {
    ...
    for (const zone of normalizedZones) {
      ...
      assertPolygonBelowSurface(zone.polygon, normalizedSurface.points, zone.id, BASE_TOLERANCE);
    }
  ...
  surfaceElevationAt(x: number | undefined): number {
    return interpolatePolyline(this.surface.points, finite(x, "x"), BASE_TOLERANCE);
  }

  getZonesAtPoint(
    { x, z }: GroundSectionPointInput = {},
    { includeBoundary = true }: GroundSectionPointQueryOptions = {},
  ): GroundSectionZone[] {
    ...
    return this.zones
      .filter((zone) => {
        ...
        const location = pointLocationInPolygon(point, zone.polygon, BASE_TOLERANCE);
  ...
  getZoneAtPoint(
    point: GroundSectionPointInput = {},
    options: GroundSectionPointQueryOptions = {},
  ): GroundSectionZone | null {
    const matches = this.getZonesAtPoint(point, options);
    if (matches.length === 0) return null;
    if (matches.length > 1 && options.requireUnique !== false) {
      throw new Error(
        `Point belongs to multiple GroundSection2D zone boundaries: ${matches
          .map(({ id }) => id)
          .join(", ")}.`,
      );
    }
  ...
  isBelowGroundSurface({ x, z }: GroundSectionPointInput = {}): boolean {
    return finite(z, "point.z") <= this.surfaceElevationAt(x);
  }

  toJSON(): GroundSection2DJson {
    return {
      ...
      zones: this.zones.map((zone) => {
        const serializableZone = structuredClone(zone);
        Reflect.deleteProperty(serializableZone, "bounds");
        return serializableZone;
      }),
      bounds: { ...this.bounds },
      units: { ...this.units },
