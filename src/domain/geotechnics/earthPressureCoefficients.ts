export interface RankineEarthPressureCoefficients {
  active: number;
  passive: number;
  basis: string;
  method: string;
...
export interface JakyAtRestCoefficient {
  coefficient: number;
  basis: string;
  method: string;
  reference: string;
...
export interface CoulombEarthPressureCoefficient {
  coefficient: number;
  normalCoefficient: number;
  tangentCoefficient: number;
  directionAngle: number;
  basis: string;
  wallInclinationFromVertical: number;
  wallInclinationPositiveDirection: string;
  backfillInclination: number;
  backfillInclinationPositiveDirection: string;
  method: string;
  reference: string;
  warnings?: string[];
}

export interface MononobeOkabeActiveEarthPressureCoefficient {
  coefficient: number;
  gravityMultiplier: number;
  equivalentCoefficient: number;
  normalCoefficient: number;
  tangentCoefficient: number;
  equivalentNormalCoefficient: number;
  equivalentTangentCoefficient: number;
  directionAngle: number;
  inertiaAngle: number;
  basis: string;
  verticalSeismicConvention: string;
  method: string;
  reference: string;
}

export interface FrictionAngleOptions {
  frictionAngle?: number;
}

export interface CoulombEarthPressureOptions extends FrictionAngleOptions {
  interfaceFrictionAngle?: number;
  wallInclinationFromVertical?: number;
  backfillInclination?: number;
}

export interface MononobeOkabeEarthPressureOptions extends FrictionAngleOptions {
  interfaceFrictionAngle?: number;
  horizontalSeismicCoefficient?: number;
  verticalSeismicCoefficient?: number;
}

function assertFrictionAngle(frictionAngle: unknown): number {
  if (
    typeof frictionAngle !== "number" ||
...
function assertInterfaceAngle(interfaceFrictionAngle: unknown, frictionAngle: number): number {
  if (typeof interfaceFrictionAngle !== "number" || !Number.isFinite(interfaceFrictionAngle)) {
    throw new Error("interfaceFrictionAngle must be finite.");
  }
  if (interfaceFrictionAngle < 0 || interfaceFrictionAngle > frictionAngle) {
    throw new Error("interfaceFrictionAngle must satisfy 0 <= delta <= frictionAngle.");
  }
  return interfaceFrictionAngle;
}

function assertWallAngle(wallInclinationFromVertical: unknown): number {
  const inclination = Number(wallInclinationFromVertical ?? 0);
  if (!Number.isFinite(inclination) || Math.abs(inclination) >= Math.PI / 2) {
    throw new Error("wallInclinationFromVertical must satisfy -pi/2 < inclination < pi/2.");
  }
  return inclination;
}

function assertBackfillAngle(backfillInclination: unknown): number {
  const inclination = Number(backfillInclination ?? 0);
  if (!Number.isFinite(inclination) || Math.abs(inclination) >= Math.PI / 2) {
    throw new Error("backfillInclination must satisfy -pi/2 < inclination < pi/2.");
  }
  return inclination;
}
...
export function rankineEarthPressureCoefficients({
  frictionAngle,
}: FrictionAngleOptions = {}): RankineEarthPressureCoefficients {
  ...
  return {
    ...
    reference: "USACE EM 1110-2-2502 (1989), chapter 3, Rankine limiting earth pressure",
  };
}

export function jakyAtRestCoefficient({
  frictionAngle,
}: FrictionAngleOptions = {}): JakyAtRestCoefficient {
  const phi = assertFrictionAngle(frictionAngle);
  return {
    coefficient: 1 - Math.sin(phi),
    basis: "horizontal-effective-stress",
    method: "jaky-normally-consolidated",
    reference: "Jaky (1944), at-rest correlation for normally consolidated soil",
  };
}
...
export function coulombActiveEarthPressureCoefficient({
...
}: CoulombEarthPressureOptions = {}): CoulombEarthPressureCoefficient {
  const phi = assertFrictionAngle(frictionAngle);
  ...
  if (slopeStrength < -1e-14) {
    throw new Error("Coulomb active pressure requires backfillInclination <= frictionAngle.");
  }

  const radical = (Math.sin(phi + delta) * Math.max(0, slopeStrength)) / (wallTerm * surfaceTerm);
  ...
  const coefficient =
    Math.sin(theta + phi) ** 2 /
    (Math.sin(theta) ** 2 * wallTerm * (1 + Math.sqrt(Math.max(0, radical))) ** 2);
  ...
  return {
    ...
    reference: "USACE EM 1110-2-2502 (1989), chapter 3, Coulomb planar-wedge theory",
  };
}
...
export function coulombPassiveEarthPressureCoefficient({
  ...
  return {
    ...
    reference: "USACE EM 1110-2-2502 (1989), sections 3-12c and 3-13d",
    warnings:
      delta > 1e-14
        ? [
...
export function mononobeOkabeActiveEarthPressureCoefficient({
...
}: MononobeOkabeEarthPressureOptions = {}): MononobeOkabeActiveEarthPressureCoefficient {
  const phi = assertFrictionAngle(frictionAngle);
  const delta = assertInterfaceAngle(interfaceFrictionAngle, phi);
  ...
  const denominator = Math.cos(theta) * cosDeltaTheta * (1 + Math.sqrt(radical)) ** 2;
  if (denominator <= 0) {
  ...
  return {
    ...
    inertiaAngle: theta,
    basis: "resultant-traction-on-wall",
    verticalSeismicConvention: "positive-kv-reduces-effective-gravity-through-factor-1-minus-kv",
    method: "mononobe-okabe-active-vertical-wall-horizontal-ground",
    reference: "USACE EM 1110-2-2100 (2005), appendix G, equations G-1 and G-2",
  };
}
