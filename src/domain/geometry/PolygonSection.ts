...
import { CrossSection, type SectionMetadata, type SectionPoint } from "./CrossSection.js";
...
export interface PolygonSectionOptions {
  points: SectionPoint[];
  id?: string | null;
  name?: string;
...
function assertPoint(point: SectionPoint | null | undefined, index: number): void {
...
export class PolygonSection extends CrossSection {
  constructor({
    ...
    const resolvedPoints = points.map(
    ...
    const inertiaY = Math.abs(inertiaYOrigin / 12 - signedArea * centroidY ** 2);
    const inertiaZ = Math.abs(inertiaZOrigin / 12 - signedArea * centroidZ ** 2);
    const massProperties = calculateSectionMassProperties({ points: resolvedPoints });
    ...
    super({
      ...
      elasticSectionModulusY: inertiaY / Math.max(maxY - centroidY, centroidY - minY),
      elasticSectionModulusZ: inertiaZ / Math.max(maxZ - centroidZ, centroidZ - minZ),
      height: maxY - minY,
      width: maxZ - minZ,
