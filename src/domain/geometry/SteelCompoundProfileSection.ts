...
// source path: src/domain/geometry/SteelCompoundProfileSection.js.

import { calculateSectionMassProperties, rotateSecondMoments } from "./SectionMassProperties.js";
import {
...
export interface SteelCompoundSectionLike {
  readonly area: number;
  readonly inertiaY: NullableNumber;
  readonly inertiaZ: NullableNumber;
  readonly productOfInertiaYZ?: NullableNumber;
  readonly torsionalConstant?: NullableNumber;
  readonly shearAreaY?: NullableNumber;
  readonly shearAreaZ?: NullableNumber;
  readonly height?: NullableNumber;
  readonly width?: NullableNumber;
  readonly webThickness?: NullableNumber;
  readonly flangeThickness?: NullableNumber;
  readonly profileName?: string | null | undefined;
  readonly family?: string | null;
  readonly name?: string | null;
  readonly massPerLength?: NullableNumber;
  readonly convertedCatalogProperties?: Record<string, unknown> | null;
  readonly metadata?: SectionMetadata | null;
}

export interface SteelCompoundComponentInput {
  readonly id?: string | null;
  readonly role?: string | null;
  readonly section?: SteelCompoundSectionLike;
  readonly profileName?: string | null;
  readonly rotation?: number;
  readonly alpha?: number;
  readonly rotationUnits?: string;
  readonly angleUnits?: string;
...
export interface SteelCompoundBounds {
  minY: number;
  maxY: number;
  minZ: number;
  maxZ: number;
...
export interface SteelCompoundComponent {
  id: string | null;
  role: string | null;
  section: SteelCompoundSectionLike;
  ...
  torsionalConstant: NullableNumber;
  shearAreaY: NullableNumber;
  shearAreaZ: NullableNumber;
  massPerLength: NullableNumber;
  transformedPoints: SectionPoint[];
  bounds: SteelCompoundBounds;
  localGeometry: {
    centroidY: number;
    centroidZ: number;
    bounds: SteelCompoundBounds;
...
export interface SteelCompoundProfileSectionOptions {
  id?: string | null;
  name?: string | null;
  components?: readonly SteelCompoundComponentInput[];
  units?: UnitSystemInput | null;
  metadata?: SectionMetadata;
}

export interface SteelCompoundProfileSectionJson extends CrossSectionJson {
  family: string;
  profileName: string | null;
...
function isFinitePositive(value: NullableNumber): value is number {
  return typeof value === "number" && Number.isFinite(value) && value > 0;
}

function angleToRadians(value = 0, units = "rad"): number {
...
function mirroredProduct(productOfInertiaYZ: number, mirrorY: boolean, mirrorZ: boolean): number {
...
function boundsFromPoints(points: readonly SectionPoint[]): SteelCompoundBounds {
  return {
...
function sectionDimensions(section: SteelCompoundSectionLike): {
  h: number | null;
  b: number | null;
  tw: number | null;
  tf: number | null;
  ...
  return {
    ...
    tw: typeof tw === "number" ? tw : null,
    tf: typeof tf === "number" ? tf : null,
  };
}

function upnOutline(section: SteelCompoundSectionLike): SectionPoint[] | null {
...
function angleOutline(section: SteelCompoundSectionLike): SectionPoint[] | null {
  const { h, b, tw, tf } = sectionDimensions(section);
  const t = Math.max(tw ?? 0, tf ?? 0);

  if (!isFinitePositive(h) || !isFinitePositive(b) || !isFinitePositive(t)) {
    return null;
  }
...
function fallbackOutline(section: SteelCompoundSectionLike): SectionPoint[] | null {
  const { h, b } = sectionDimensions(section);

  if (!isFinitePositive(h) || !isFinitePositive(b)) {
    return null;
  }
...
function nominalOutline(section: SteelCompoundSectionLike): SectionPoint[] | null {
...
interface CenteredNominalGeometry {
  centroidY: number;
  centroidZ: number;
  productOfInertiaYZ: number;
  points: SectionPoint[];
...
function centeredNominalGeometry(section: SteelCompoundSectionLike): CenteredNominalGeometry {
  ...
  if (!outline) {
    ...
    if (!isFinitePositive(h) || !isFinitePositive(b)) {
      throw new Error(
        `Compound steel component ${section.profileName ?? section.name} requires height and width.`,
      );
    }

    const points = fallbackOutline(section);
    if (!points) {
      throw new Error(
        `Compound steel component ${section.profileName ?? section.name} requires height and width.`,
      );
  ...
  const centroidY = properties.centroidY ?? 0;
  const centroidZ = properties.centroidZ ?? 0;
  const centeredPoints: SectionPoint[] = outline.map((point) => ({
    y: point.y - centroidY,
    z: point.z - centroidZ,
  }));
...
function localProductOfInertia(
  section: SteelCompoundSectionLike,
  geometry: CenteredNominalGeometry,
): number {
  return finiteOrZero(
...
function resolveComponentSection(
  component: SteelCompoundComponentInput,
  defaultUnits: UnitSystemInput | null | undefined,
...
function normalizeComponent(
  ...
  const localProduct = mirroredProduct(localProductOfInertia(section, geometry), mirrorY, mirrorZ);
  const rotated = rotateSecondMoments({
    inertiaY: section.inertiaY ?? Number.NaN,
    inertiaZ: section.inertiaZ ?? Number.NaN,
  ...
  return {
    ...
    productOfInertiaYZ:
      Math.abs(rotated.productOfInertiaYZ) <= PRODUCT_TOLERANCE ? 0 : rotated.productOfInertiaYZ,
...
function maxDistance(values: readonly number[], centroid: number): number {
  return Math.max(...values.map((value) => Math.abs(value - centroid)));
}

interface SteelCompoundProperties {
  area: number;
  centroidY: number;
  centroidZ: number;
  inertiaY: number;
...
function calculateCompoundProperties(
  components: readonly SteelCompoundComponent[],
): SteelCompoundProperties {
  ...
  const centroidY =
    components.reduce((sum, component) => sum + component.area * component.centroidY, 0) / area;
  const centroidZ =
    components.reduce((sum, component) => sum + component.area * component.centroidZ, 0) / area;
  const inertiaY = components.reduce(
    (sum, component) =>
      sum + component.inertiaY + component.area * (component.centroidY - centroidY) ** 2,
    0,
  );
  const inertiaZ = components.reduce(
    (sum, component) =>
      sum + component.inertiaZ + component.area * (component.centroidZ - centroidZ) ** 2,
    0,
  );
  const productOfInertiaYZ = components.reduce(
    (sum, component) =>
      ...
      component.area * (component.centroidY - centroidY) * (component.centroidZ - centroidZ),
    0,
  );
  ...
  return {
    ...
    inertiaY,
    inertiaZ,
    productOfInertiaYZ: Math.abs(productOfInertiaYZ) <= PRODUCT_TOLERANCE ? 0 : productOfInertiaYZ,
    torsionalConstant: components.reduce(
      (sum, component) => sum + finiteOrZero(component.torsionalConstant),
      0,
    ),
    shearAreaY: components.reduce((sum, component) => sum + finiteOrZero(component.shearAreaY), 0),
    shearAreaZ: components.reduce((sum, component) => sum + finiteOrZero(component.shearAreaZ), 0),
...
function isComponentArray(
  value: readonly SteelCompoundComponentInput[] | undefined,
): value is readonly SteelCompoundComponentInput[] {
  return Array.isArray(value);
}

export class SteelCompoundProfileSection extends CrossSection {
  declare components: SteelCompoundComponent[];
  declare family: string;
  declare profileName: string | null;
  declare massPerLength: number;
  declare bounds: SteelCompoundBounds;
  ...
  constructor({
    ...
    if (!isComponentArray(components) || components.length === 0) {
    ...
    this.profileName = id ?? name ?? null;
    this.massPerLength = properties.massPerLength;
    this.bounds = { ...properties.bounds };
  }

  getComponent(role: string): SteelCompoundComponent | null {
    return this.components.find((component) => component.role === role) ?? null;
  }

  override toJSON(): SteelCompoundProfileSectionJson {
    return {
      ...super.toJSON(),
      family: this.family,
...
export function createSteelCompoundProfileSection(
  options: SteelCompoundProfileSectionOptions,
): SteelCompoundProfileSection {
  return new SteelCompoundProfileSection(options);
}

export interface DoubleUPNBackToBackOptions {
  profileName?: string | null;
  gap?: number;
  webGap?: number;
  units?: UnitSystemInput | null;
...
export function createDoubleUPNBackToBackSection({
  ...
  name = null,
  metadata = {},
}: DoubleUPNBackToBackOptions = {}): SteelCompoundProfileSection {
...
export interface DoubleAngleOpposedOptions {
  profileName?: string | null;
  separationY?: number;
  separationZ?: number;
  gapY?: number;
...
export function createDoubleAngleOpposedSection({
  ...
  name = null,
  metadata = {},
}: DoubleAngleOpposedOptions = {}): SteelCompoundProfileSection {
  ...
  if (!Number.isFinite(resolvedSeparationY) || !Number.isFinite(resolvedSeparationZ)) {
