...
export interface TSectionOptions {
  flangeWidth: number;
...
export interface TSectionJson extends CrossSectionJson {
  flangeWidth: number;
  flangeThickness: number;
  webWidth: number;
  webHeight: number;
...
export class TSection extends CrossSection {
  ...
  constructor({
    ...
    const centroidY = (flangeArea * flangeCentroidY + webArea * webCentroidY) / area;
    const inertiaY =
    ...
    super({
      ...
      inertiaY,
      inertiaZ,
      elasticSectionModulusY: inertiaY / Math.max(totalHeight - centroidY, centroidY),
