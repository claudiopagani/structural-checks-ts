// Mechanical TypeScript migration from strutture-js 6f33baead8b88166c4b2cf94af41763412e3c751; source path: src/domain/connectors/TimberDowelConnector.js.
...
export interface TimberDowelConnectorOptions extends Record<string, unknown> {
  id?: unknown;
  name?: string | null;
  diameter: number;
  timberDensityMean: number;
  timberDensityCharacteristicSection1: number;
  timberDensityCharacteristicSection2: number;
...
export interface TimberDowelCharacteristicResistance {
  rk1a: number;
  rk1b: number;
  rk1c: number;
  rk2a: number;
...
export interface TimberDowelConnectorJson extends ShearConnectorJson {
  diameter: number;
  timberDensityMean: number;
  timberDensityCharacteristicSection1: number;
  timberDensityCharacteristicSection2: number;
...
export class TimberDowelConnector extends ShearConnector {
  ...
  timberTimberCharacteristicResistance(
    section1Thickness: number,
    ...
    const rk1c =
      (((fhk1 * d * t1) / (1 + beta)) *
        (Math.sqrt(
          beta + 2 * beta ** 2 * (1 + t2 / t1 + (t2 / t1) ** 2) + beta ** 3 * (t2 / t1) ** 2,
        ) -
          beta * (1 + t2 / t1))) /
      1000;
    const rk2a =
      (((1.1 * fhk1 * d * t1) / (2 + beta)) *
        (Math.sqrt(2 * beta * (1 + beta) + (4 * beta * (2 + beta) * my) / (fhk1 * d * t1 ** 2)) -
          beta)) /
      1000;
    const rk2b =
      (((1.1 * fhk1 * d * t2) / (1 + 2 * beta)) *
        (Math.sqrt(
          2 * beta ** 2 * (1 + beta) + (4 * beta * (2 + beta) * my) / (fhk1 * d * t2 ** 2),
        ) -
          beta)) /
      1000;
    const rk3 = (1.15 * Math.sqrt((2 * beta) / (1 + beta)) * Math.sqrt(2 * my * fhk1 * d)) / 1000;
  ...
  override toJSON(): TimberDowelConnectorJson {
    return {
      ...super.toJSON(),
