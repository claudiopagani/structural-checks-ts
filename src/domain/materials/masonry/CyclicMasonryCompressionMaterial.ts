...
export const CYCLIC_MASONRY_INTERNAL_UNITS = Object.freeze({ force: "kN", length: "m" });
const INTERNAL_UNITS = CYCLIC_MASONRY_INTERNAL_UNITS;
const STATE_VERSION = 1;
const DIRECTION_TOLERANCE = 1e-14;

type CompressionBranch =
  | "initial"
  | "open"
  | "tension-elastic"
  | "tension-cracked"
...
export interface CyclicMasonryCompressionState {
  version: number;
  strain: number;
  stress: number;
  tangent: number;
  ...
  failed: boolean;
  branch: CompressionBranch;
}

export interface CyclicMasonryCompressionMaterialOptions {
  id?: string | null;
  ...
  damageOnsetStrain?: number;
  unloadingStiffnessDegradation?: number;
  strengthDegradation?: number;
  energyDamageCoefficient?: number;
  energyCapacity?: number | null;
  tensileStrength?: number;
  tensionElasticModulus?: number;
  tensionFractureEnergy?: number | null;
  compressionFractureEnergy?: number | null;
  hingeLength: number;
  numericalTangentRatio?: number;
  metadata?: Record<string, unknown>;
}

export interface CyclicMasonryCompressionConfiguration extends Record<string, unknown> {
  id: string | null;
  units: UnitSystem;
  elasticModulus: number;
  compressiveStrength: number;
...
export interface CyclicMasonryCompressionMaterialJson {
  type: string;
  configuration: CyclicMasonryCompressionConfiguration;
  committedState: CyclicMasonryCompressionState;
  trialState: CyclicMasonryCompressionState;
}

interface CompressionEnvelopeResponse {
  stress: number;
  tangent: number;
}

interface TensionResponse extends CompressionEnvelopeResponse {
  branch: CompressionBranch;
}

function assertPositive(value: number, label: string): void {
  if (!Number.isFinite(value) || value <= 0) {
    throw new Error(`CyclicMasonryCompressionMaterial requires a positive ${label}.`);
  }
}

function assertNonNegative(value: number, label: string): void {
  if (!Number.isFinite(value) || value < 0) {
    throw new Error(`CyclicMasonryCompressionMaterial requires a non-negative ${label}.`);
  }
}

function assertRatio(value: number, label: string): void {
  if (!Number.isFinite(value) || value < 0 || value > 1) {
    throw new Error(`CyclicMasonryCompressionMaterial requires ${label} between 0 and 1.`);
  }
}
...
function loadingDirection(increment: number, fallback = 0): number {
...
function cloneState(state: CyclicMasonryCompressionState): CyclicMasonryCompressionState {
  return { ...state };
}
...
export class CyclicMasonryCompressionMaterial {
  ...
  readonly prePeakCurve: string;
  readonly residualStrengthRatio: number;
  readonly damageOnsetStrain: number;
  readonly unloadingStiffnessDegradation: number;
  readonly strengthDegradation: number;
  readonly energyDamageCoefficient: number;
  readonly tensileStrength: number;
  readonly tensionElasticModulus: number;
  readonly tensionFractureEnergy: number | null;
  readonly compressionFractureEnergy: number | null;
  readonly hingeLength: number;
  readonly numericalTangentRatio: number;
  readonly metadata: Record<string, unknown>;
  readonly ultimateStrain: number;
  readonly energyCapacity: number;
  private _configuration: CyclicMasonryCompressionConfiguration;
  private _committedState: CyclicMasonryCompressionState;
  private _trialState: CyclicMasonryCompressionState;
  ...
  constructor({
    ...
    this.tensionFractureEnergy =
      tensionFractureEnergy == null
        ? null
        : resolver.convert(tensionFractureEnergy, {
            forceExponent: 1,
            lengthExponent: -1,
          });
    this.compressionFractureEnergy =
      compressionFractureEnergy == null
        ? null
        : resolver.convert(compressionFractureEnergy, {
            forceExponent: 1,
            lengthExponent: -1,
          });
    ...
    assertRatio(this.unloadingStiffnessDegradation, "unloadingStiffnessDegradation");
    ...
    if (!new Set(["linear", "cubic"]).has(this.prePeakCurve)) {
      throw new Error('CyclicMasonryCompressionMaterial prePeakCurve must be "linear" or "cubic".');
    }

    if (!Number.isFinite(this.damageOnsetStrain) || this.damageOnsetStrain < this.peakStrain) {
      throw new Error("CyclicMasonryCompressionMaterial requires damageOnsetStrain >= peakStrain.");
    }

    if (
      this.compressionFractureEnergy != null &&
    ...
    if (
      this.tensionFractureEnergy != null &&
      Number.isFinite(this.tensionFractureEnergy) &&
      this.tensionFractureEnergy <= 0
    ) {
      throw new Error("CyclicMasonryCompressionMaterial tensionFractureEnergy must be positive.");
    }

    const derivedUltimateStrain = this.deriveUltimateStrainFromEnergy();
    const candidateUltimateStrain = ultimateStrain ?? derivedUltimateStrain;
    if (candidateUltimateStrain == null) {
      throw new Error(
    ...
    this.ultimateStrain = candidateUltimateStrain;

    if (!Number.isFinite(this.ultimateStrain) || this.ultimateStrain <= this.damageOnsetStrain) {
      throw new Error(
        "CyclicMasonryCompressionMaterial requires ultimateStrain > damageOnsetStrain, or compressionFractureEnergy from which it can be derived.",
      );
    }
    ...
    this.energyCapacity =
      resolver.stress(energyCapacity) ?? this.compressiveStrength * this.ultimateStrain;
    ...
    if (this.prePeakCurve === "linear" && Math.abs(normalizedInitialSlope - 1) > 1e-9) {
      throw new Error(
  ...
  deriveUltimateStrainFromEnergy(): number | null {
    if (
      this.compressionFractureEnergy == null ||
      !Number.isFinite(this.compressionFractureEnergy)
    ) {
    ...
    const strengthDrop = this.compressiveStrength * (1 - this.residualStrengthRatio);
    ...
    return (
      this.peakStrain + (2 * this.compressionFractureEnergy) / (this.hingeLength * strengthDrop)
    );
  }
  ...
  initialState(): CyclicMasonryCompressionState {
    return {
  ...
  compressionEnvelope(compressionStrain: number, strengthDamage = 0): CompressionEnvelopeResponse {
    const x = Math.max(0, compressionStrain);
    const residualStrength = this.residualStrengthRatio * this.compressiveStrength;
    let baseStress;
    let baseTangent;
    ...
    if (x <= this.peakStrain) {
      if (this.prePeakCurve === "linear") {
        ...
        const slopeRatio = (this.elasticModulus * this.peakStrain) / this.compressiveStrength;
        const a = slopeRatio - 2;
        const b = 3 - 2 * slopeRatio;

        baseStress =
          this.compressiveStrength * (a * ratio ** 3 + b * ratio ** 2 + slopeRatio * ratio);
        baseTangent =
          (this.compressiveStrength / this.peakStrain) *
          (3 * a * ratio ** 2 + 2 * b * ratio + slopeRatio);
      }
    } else if (x < this.ultimateStrain) {
      const postPeakRatio = (x - this.peakStrain) / (this.ultimateStrain - this.peakStrain);
      baseStress =
        this.compressiveStrength - postPeakRatio * (this.compressiveStrength - residualStrength);
      baseTangent =
        -(this.compressiveStrength - residualStrength) / (this.ultimateStrain - this.peakStrain);
    } else {
    ...
    const degradedStress =
      x <= this.peakStrain
        ...
        : Math.max(residualStrength, (1 - strengthDamage) * baseStress);
    const tangentScale =
      degradedStress === residualStrength && baseStress > 0 ? 0 : 1 - strengthDamage;
  ...
  damageFor(
    ...
    crushingProgress: number;
    strengthDamage: number;
    stiffnessDamage: number;
    damage: number;
  } {
    const crushingRange = this.ultimateStrain - this.damageOnsetStrain;
    const crushingProgress = clamp((compressionStrain - this.damageOnsetStrain) / crushingRange);
    const energyProgress = Math.max(0, dissipatedEnergy / this.energyCapacity);
    const strengthDamage = clamp(
      this.strengthDegradation * crushingProgress + this.energyDamageCoefficient * energyProgress,
      0,
  ...
  tensionResponse(openingStrain: number): TensionResponse {
    ...
    const crackingStrain = this.tensileStrength / this.tensionElasticModulus;
    ...
    if (this.tensionFractureEnergy == null || !Number.isFinite(this.tensionFractureEnergy)) {
      return {
        stress: 0,
        tangent: this.numericalTangentRatio * this.elasticModulus,
    ...
    const softeningRange =
      (2 * this.tensionFractureEnergy) / (this.tensileStrength * this.hingeLength);
    const softeningProgress = (openingStrain - crackingStrain) / softeningRange;
  ...
  setTrialStrain(strain: number): number {
    if (!Number.isFinite(strain)) {
      throw new Error("CyclicMasonryCompressionMaterial setTrialStrain requires a finite strain.");
    }
    ...
    const newMaximum = compressionStrain > committed.maxCompressionStrain + DIRECTION_TOLERANCE;
    let response: TensionResponse;
    ...
    if (newMaximum) {
      ...
      crushingActivated = crushingActivated || compressionStrain > this.damageOnsetStrain;
      const nextDamage = this.damageFor(compressionStrain, committed.dissipatedEnergy);
      ...
      const envelope = this.compressionEnvelope(compressionStrain, strengthDamage);
      maxCompressionStress = envelope.stress;

      if (!crushingActivated) {
        unloadingStiffness =
          compressionStrain > 0 ? envelope.stress / compressionStrain : this.elasticModulus;
        zeroStressStrain = 0;
      } else {
        ...
        zeroStressStrain = strain + envelope.stress / unloadingStiffness;
      }
      ...
      if (openingStrain >= 0) {
        ...
        response = {
          ...
          tangent: boundedStress === lineStress ? unloadingStiffness : envelope.tangent,
          branch: direction < 0 ? "compression-reloading" : "compression-unloading",
        };
    ...
    const workIncrement = 0.5 * (committed.stress + response.stress) * increment;
    ...
    const elasticStrain =
      response.stress === 0
        ? 0
        : response.stress / (response.stress < 0 ? unloadingStiffness : this.tensionElasticModulus);
  ...
  importState(
    ...
    if (!state || state.version !== STATE_VERSION) {
      throw new Error(`CyclicMasonryCompressionMaterial requires state version ${STATE_VERSION}.`);
    }
  ...
  clone(): CyclicMasonryCompressionMaterial {
  ...
  toJSON(): CyclicMasonryCompressionMaterialJson {
    return {
      type: this.type,
