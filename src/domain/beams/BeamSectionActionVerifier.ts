import {
  VerificationResult,
  type VerificationCheck,
} from "../../core/results/VerificationResult.js";
import { RESULT_STATUS, type ResultStatus } from "../../core/results/resultStatus.js";

export interface BeamPrincipalActions extends Record<string, unknown> {
  vY?: number;
  vZ?: number;
  mY?: number;
  mZ?: number;
}

export interface BeamInternalForceSample extends Record<string, unknown> {
  station: number;
  n?: number;
  v?: number;
  m?: number;
  t?: number;
  tEd?: number;
  torsion?: number;
  vY?: number;
  vZ?: number;
  mY?: number;
  mZ?: number;
  principalActions?: BeamPrincipalActions | null;
}

export interface BeamResultEntry extends Record<string, unknown> {
  id: string;
  resultType?: string;
  geometry?: {
    length?: number;
    horizontalSpan?: number;
    [key: string]: unknown;
  };
  supports?: BeamStationInput[];
  loads?: Array<BeamStationInput & { type?: string }>;
  internalForces?: {
    samples?: BeamInternalForceSample[];
    maxAbsBendingMoment?: { station?: number };
    maxShearForce?: { station?: number };
    minShearForce?: { station?: number };
    [key: string]: unknown;
  };
  context?: {
    limitState?: string | null;
    combinationType?: string | null;
    [key: string]: unknown;
  };
  sectionProperties?: unknown;
  units?: unknown;
}

export interface BeamAnalysisResult extends Record<string, unknown> {
  combinations?: Record<string, BeamResultEntry>;
  loadCases?: Record<string, BeamResultEntry>;
  units?: unknown;
}

export interface BeamStationDescriptor extends Record<string, unknown> {
  id?: string;
  label?: string;
  station?: number | string | null;
  position?: number | string | null;
  x?: number | string | null;
  value?: number | string | null;
  at?: number | string | null;
}

export type BeamStationInput = number | string | BeamStationDescriptor;

export interface BeamVerificationStationOptions extends Record<string, unknown> {
  enabled?: boolean;
  mode?: string;
  userStations?: BeamStationInput | BeamStationInput[] | undefined;
  stations?: BeamStationInput | BeamStationInput[] | undefined;
  checkStations?: BeamStationInput | BeamStationInput[] | undefined;
  count?: number | null;
  stationCount?: number | null;
  tolerance?: number;
  stationTolerance?: number;
}

export type BeamVerificationStations = BeamStationInput[] | BeamVerificationStationOptions | null;

export interface BeamStationMetadata extends Record<string, unknown> {
  stationSource: string;
  stationRole: string;
  stationRoles: string[];
  stationSelectionMode: string;
  isRequestedStation: boolean;
  isUserStation: boolean;
  isGridStation: boolean;
  isCriticalStation: boolean;
  requestedStationIds: string[];
  stationTolerance: number;
}

export interface BeamSectionActionContext extends Record<string, unknown> {
  analysisResult?: BeamAnalysisResult;
  result?: BeamResultEntry;
  resultId?: string;
  resultType?: string | undefined;
  limitState?: string | null;
  combinationType?: string | null;
  stationMetadata?: BeamStationMetadata;
  sectionProperties?: unknown;
  units?: unknown;
}

export interface BeamSectionActionInput extends Record<string, unknown> {
  nEd?: number | undefined;
  vEd?: number | undefined;
  mEd?: number | undefined;
  tEd?: number;
  principalActions?: BeamPrincipalActions | null;
  vYEd?: number | null;
  vZEd?: number | null;
  mYEd?: number | null;
  mZEd?: number | null;
  x?: number;
  station?: number;
  sample?: BeamInternalForceSample;
  context?: BeamSectionActionContext;
}

export interface BeamSectionActionVerification extends Record<string, unknown> {
  status?: ResultStatus;
  utilizationRatio?: number | null;
  demand?: unknown;
  capacity?: unknown;
  checks?: VerificationCheck[];
  warnings?: string[];
  assumptions?: string[];
  metadata?: Record<string, unknown>;
}

export type BeamSectionActionVerificationFunction = (
  input: BeamSectionActionInput,
) => BeamSectionActionVerification;

export interface BeamSectionActionVerificationProvider {
  verifySectionActions: BeamSectionActionVerificationFunction;
}

export type BeamSectionVerifier =
  | BeamSectionActionVerificationFunction
  | BeamSectionActionVerificationProvider;

export interface BeamSectionActionVerifierOptions {
  applicationId?: string;
  sectionVerifier?: BeamSectionVerifier | null;
  includeLoadCases?: boolean;
  limitStates?: string | string[] | Set<string> | null;
  verificationStations?: BeamVerificationStations;
  metadata?: Record<string, unknown>;
}

export interface BeamSectionActionVerifyInput {
  analysisResult?: BeamAnalysisResult | null;
  sectionVerifier?: BeamSectionVerifier | null;
  includeLoadCases?: boolean;
  limitStates?: string | string[] | Set<string> | null;
  verificationStations?: BeamVerificationStations;
  context?: Record<string, unknown>;
}

interface RequestedStation {
  station: number;
  source: "user" | "grid";
  label: string;
  raw: BeamStationInput | null;
}

interface ResolvedRequestedStation {
  station: number | null;
  source: "user";
  label: string;
  raw: BeamStationInput;
  warning?: string;
}

interface StationSettings {
  enabled: boolean;
  mode: string;
  tolerance: number;
  requestedStations: RequestedStation[];
  warnings: string[];
  raw: BeamVerificationStations;
}

interface NormalizedActionVerification {
  status: ResultStatus;
  utilizationRatio: number | null;
  demand: unknown;
  capacity: unknown;
  checks: VerificationCheck[];
  warnings: string[];
  assumptions: string[];
  metadata: Record<string, unknown>;
}

function isFiniteNumber(value: unknown): value is number {
  return Number.isFinite(value);
}

function entriesFromResults(
  analysisResult: BeamAnalysisResult,
  includeLoadCases: boolean,
): BeamResultEntry[] {
  const combinations = Object.values(analysisResult.combinations ?? {});

  if (combinations.length > 0) {
    return combinations;
  }

  return includeLoadCases ? Object.values(analysisResult.loadCases ?? {}) : [];
}

function normalizeLimitStates(
  limitStates: string | string[] | Set<string> | null | undefined,
): Set<string> | null {
  if (limitStates == null) {
    return null;
  }

  const values =
    limitStates instanceof Set
      ? [...limitStates]
      : Array.isArray(limitStates)
        ? limitStates
        : [limitStates];

  return new Set(values.map((limitState) => String(limitState).toUpperCase()));
}

function copyVerificationStations(
  verificationStations: BeamVerificationStations | undefined,
): BeamVerificationStations {
  if (Array.isArray(verificationStations)) {
    return [...verificationStations];
  }

  if (!verificationStations || typeof verificationStations !== "object") {
    return verificationStations ?? null;
  }

  return {
    ...verificationStations,
    userStations: Array.isArray(verificationStations.userStations)
      ? [...verificationStations.userStations]
      : verificationStations.userStations,
    stations: Array.isArray(verificationStations.stations)
      ? [...verificationStations.stations]
      : verificationStations.stations,
    checkStations: Array.isArray(verificationStations.checkStations)
      ? [...verificationStations.checkStations]
      : verificationStations.checkStations,
  };
}

function normalizeStationMode(mode: string | null | undefined): string {
  const normalized = String(mode ?? "all")
    .trim()
    .toLowerCase();
  const aliases: Record<string, string> = {
    automatic: "auto",
    declared: "combined",
    grid: "auto",
    selected: "combined",
    fem: "all",
    samples: "all",
  };

  return aliases[normalized] ?? normalized;
}

function rawStationValues(options: BeamVerificationStationOptions): BeamStationInput[] {
  const raw = options.userStations ?? options.stations ?? options.checkStations ?? [];

  if (raw == null) {
    return [];
  }

  return Array.isArray(raw) ? raw : [raw];
}

function stationValue(station: BeamStationInput): number | string | null {
  if (station && typeof station === "object") {
    return station.station ?? station.position ?? station.x ?? station.value ?? station.at ?? null;
  }

  return station;
}

function stationLabel(station: BeamStationInput, source: string, index: number): string {
  if (station && typeof station === "object") {
    return station.id ?? station.label ?? `${source}-${index + 1}`;
  }

  return `${source}-${index + 1}`;
}

function entrySpan(entry: BeamResultEntry): number | null {
  return entry.geometry?.length ?? entry.geometry?.horizontalSpan ?? null;
}

function resolveRequestedStation(
  station: BeamStationInput,
  entry: BeamResultEntry,
  source: "user",
  index: number,
): ResolvedRequestedStation {
  const span = entrySpan(entry);
  const raw = stationValue(station);
  const label = stationLabel(station, source, index);

  if (raw === "start") {
    return { station: 0, source, label, raw: station };
  }

  if (raw === "end" || raw === "span" || raw === "length") {
    return { station: span, source, label, raw: station };
  }

  const value = Number(raw);

  if (!Number.isFinite(value)) {
    return {
      station: null,
      source,
      label,
      raw: station,
      warning: `Verification station ${label} is not numeric and was ignored.`,
    };
  }

  if (Number.isFinite(span) && (value < -1e-12 || value > (span as number) + 1e-12)) {
    return {
      station: null,
      source,
      label,
      raw: station,
      warning: `Verification station ${label} at x=${value} lies outside the beam span and was ignored.`,
    };
  }

  return {
    station: Number.isFinite(span) ? Math.min(span as number, Math.max(0, value)) : value,
    source,
    label,
    raw: station,
  };
}

function stationTolerance(
  entry: BeamResultEntry,
  options: BeamVerificationStationOptions = {},
): number {
  const tolerance = Number(options.tolerance ?? options.stationTolerance);

  if (Number.isFinite(tolerance) && tolerance > 0) {
    return tolerance;
  }

  const span = entrySpan(entry);

  return Math.max(Math.abs(span ?? 0) * 1e-8, 1e-9);
}

function sameStation(a: unknown, b: unknown, tolerance: number): boolean {
  return (
    Number.isFinite(a) && Number.isFinite(b) && Math.abs((a as number) - (b as number)) <= tolerance
  );
}

function normalizeVerificationStationSettings(
  verificationStations: BeamVerificationStations | undefined,
  entry: BeamResultEntry,
): StationSettings {
  if (
    !verificationStations ||
    (!Array.isArray(verificationStations) && verificationStations.enabled === false)
  ) {
    return {
      enabled: false,
      mode: "all",
      tolerance: stationTolerance(entry),
      requestedStations: [],
      warnings: [],
      raw: null,
    };
  }

  const options: BeamVerificationStationOptions = Array.isArray(verificationStations)
    ? { mode: "user", userStations: verificationStations }
    : verificationStations;
  const explicitStations = rawStationValues(options);
  const hasDeclaredStations =
    explicitStations.length > 0 || options.count != null || options.stationCount != null;
  const mode = normalizeStationMode(options.mode ?? (hasDeclaredStations ? "combined" : "all"));
  const tolerance = stationTolerance(entry, options);
  const requestedStations: RequestedStation[] = [];
  const warnings: string[] = [];
  const count = options.count ?? options.stationCount ?? null;
  const span = entrySpan(entry);

  if (count != null && (mode === "auto" || mode === "combined")) {
    if (!Number.isInteger(count) || count < 2) {
      warnings.push("verificationStations.count must be an integer greater than or equal to 2.");
    } else if (!Number.isFinite(span)) {
      warnings.push(
        "verificationStations.count was ignored because the beam span is not available.",
      );
    } else {
      for (let index = 0; index < count; index += 1) {
        requestedStations.push({
          station: ((span as number) * index) / (count - 1),
          source: "grid",
          label: `grid-${index + 1}`,
          raw: null,
        });
      }
    }
  }

  if (mode === "user" || mode === "combined") {
    explicitStations.forEach((station, index) => {
      const resolved = resolveRequestedStation(station, entry, "user", index);

      if (resolved.warning !== undefined) {
        warnings.push(resolved.warning);
      }

      if (isFiniteNumber(resolved.station)) {
        requestedStations.push({
          ...resolved,
          station: resolved.station,
        });
      }
    });
  }

  return {
    enabled: true,
    mode,
    tolerance,
    requestedStations,
    warnings,
    raw: copyVerificationStations(verificationStations),
  };
}

function stationFromItem(item: BeamStationInput, span: number | null): number | null {
  const raw = stationValue(item);

  if (raw === "start") {
    return 0;
  }

  if (raw === "end" || raw === "span" || raw === "length") {
    return span;
  }

  const value = Number(raw);

  return Number.isFinite(value) ? value : null;
}

function supportMatches(
  entry: BeamResultEntry,
  sample: BeamInternalForceSample,
  tolerance: number,
): boolean {
  const span = entrySpan(entry);

  return (entry.supports ?? []).some((support) =>
    sameStation(stationFromItem(support, span), sample.station, tolerance),
  );
}

function pointLoadMatches(
  entry: BeamResultEntry,
  sample: BeamInternalForceSample,
  tolerance: number,
): boolean {
  const span = entrySpan(entry);

  return (entry.loads ?? []).some((load) => {
    const type = String(load.type ?? "").toLowerCase();

    if (!["point", "nodal", "force", "moment"].includes(type)) {
      return false;
    }

    return sameStation(stationFromItem(load, span), sample.station, tolerance);
  });
}

function criticalMatches(
  entry: BeamResultEntry,
  sample: BeamInternalForceSample,
  tolerance: number,
): string[] {
  const forces = entry.internalForces ?? {};
  const matches: string[] = [];

  if (sameStation(forces.maxAbsBendingMoment?.station, sample.station, tolerance)) {
    matches.push("critical-bending");
  }

  if (
    sameStation(forces.maxShearForce?.station, sample.station, tolerance) ||
    sameStation(forces.minShearForce?.station, sample.station, tolerance)
  ) {
    matches.push("critical-shear");
  }

  return matches;
}

function unique(values: string[]): string[] {
  return [...new Set(values)];
}

function sampleStationMetadata(
  entry: BeamResultEntry,
  sample: BeamInternalForceSample,
  settings: StationSettings,
): BeamStationMetadata {
  const requestedMatches = settings.requestedStations.filter((station) =>
    sameStation(station.station, sample.station, settings.tolerance),
  );
  const criticalRoles = criticalMatches(entry, sample, settings.tolerance);
  const roles: string[] = [];

  if (requestedMatches.some((station) => station.source === "user")) {
    roles.push("verification-user");
  }

  if (requestedMatches.some((station) => station.source === "grid")) {
    roles.push("verification-grid");
  }

  if (supportMatches(entry, sample, settings.tolerance)) {
    roles.push("support");
  }

  if (pointLoadMatches(entry, sample, settings.tolerance)) {
    roles.push("point-load");
  }

  roles.push(...criticalRoles);

  if (roles.length === 0) {
    roles.push("fem-sample");
  }

  const uniqueRoles = unique(roles);
  const isRequestedStation = requestedMatches.length > 0;
  const isCriticalStation = criticalRoles.length > 0;
  const isUserStation = requestedMatches.some((station) => station.source === "user");
  const isGridStation = requestedMatches.some((station) => station.source === "grid");
  const stationSource = isUserStation
    ? "user"
    : isGridStation
      ? "grid"
      : isCriticalStation
        ? "critical"
        : uniqueRoles.includes("support")
          ? "support"
          : uniqueRoles.includes("point-load")
            ? "point-load"
            : "fem-sample";

  return {
    stationSource,
    stationRole: uniqueRoles.join("+"),
    stationRoles: uniqueRoles,
    stationSelectionMode: settings.mode,
    isRequestedStation,
    isUserStation,
    isGridStation,
    isCriticalStation,
    requestedStationIds: requestedMatches.map((station) => station.label),
    stationTolerance: settings.tolerance,
  };
}

function selectVerificationSamples(
  entry: BeamResultEntry,
  settings: StationSettings,
): {
  records: Array<{ sample: BeamInternalForceSample; stationMetadata: BeamStationMetadata }>;
  warnings: string[];
} {
  const samples = entry.internalForces?.samples ?? [];
  const records = samples.map((sample) => ({
    sample,
    stationMetadata: sampleStationMetadata(entry, sample, settings),
  }));
  const warnings = [...settings.warnings];
  const selectDeclared =
    (settings.mode === "user" || settings.mode === "combined" || settings.mode === "auto") &&
    settings.requestedStations.length > 0;

  for (const station of settings.requestedStations) {
    const found = records.some((record) =>
      sameStation(record.sample.station, station.station, settings.tolerance),
    );

    if (!found) {
      warnings.push(
        `Verification station ${station.label} at x=${station.station} was requested but no FEM sample was found.`,
      );
    }
  }

  if (!settings.enabled || settings.mode === "all") {
    return { records, warnings };
  }

  if (settings.mode === "critical") {
    const selected = records.filter((record) => record.stationMetadata.isCriticalStation);

    if (selected.length === 0 && samples.length > 0) {
      warnings.push(`No critical FEM station was selected for result ${entry.id}.`);
    }

    return { records: selected, warnings };
  }

  if (selectDeclared) {
    const selected = records.filter((record) => record.stationMetadata.isRequestedStation);

    if (selected.length === 0 && samples.length > 0) {
      warnings.push(`No requested verification station was selected for result ${entry.id}.`);
    }

    return { records: selected, warnings };
  }

  return { records, warnings };
}

function normalizeActionVerification(
  raw: BeamSectionActionVerification | null | undefined,
  fallback: Record<string, unknown> = {},
): NormalizedActionVerification {
  const checks = raw?.checks ?? [];
  const utilizationRatio =
    raw?.utilizationRatio ??
    checks.reduce<number | null>(
      (max, check) =>
        isFiniteNumber(check.utilizationRatio) && (max === null || check.utilizationRatio > max)
          ? check.utilizationRatio
          : max,
      null,
    );

  return {
    status:
      raw?.status ??
      (isFiniteNumber(utilizationRatio) && utilizationRatio <= 1
        ? RESULT_STATUS.OK
        : RESULT_STATUS.NOT_VERIFIED),
    utilizationRatio,
    demand: raw?.demand ?? null,
    capacity: raw?.capacity ?? null,
    checks,
    warnings: raw?.warnings ?? [],
    assumptions: raw?.assumptions ?? [],
    metadata: {
      ...fallback,
      ...raw?.metadata,
    },
  };
}

function governingResult(
  results: NormalizedActionVerification[],
): NormalizedActionVerification | null {
  return results.reduce<NormalizedActionVerification | null>((selected, current) => {
    if (!isFiniteNumber(current.utilizationRatio)) {
      return selected;
    }

    if (
      selected === null ||
      !isFiniteNumber(selected.utilizationRatio) ||
      current.utilizationRatio > selected.utilizationRatio
    ) {
      return current;
    }

    return selected;
  }, null);
}

export class BeamSectionActionVerifier {
  applicationId: string;
  sectionVerifier: BeamSectionVerifier | null;
  includeLoadCases: boolean;
  limitStates: Set<string> | null;
  verificationStations: BeamVerificationStations;
  metadata: Record<string, unknown>;

  constructor({
    applicationId = "beam-section-actions",
    sectionVerifier = null,
    includeLoadCases = true,
    limitStates = null,
    verificationStations = null,
    metadata = {},
  }: BeamSectionActionVerifierOptions = {}) {
    this.applicationId = applicationId;
    this.sectionVerifier = sectionVerifier;
    this.includeLoadCases = includeLoadCases;
    this.limitStates = normalizeLimitStates(limitStates);
    this.verificationStations = copyVerificationStations(verificationStations);
    this.metadata = { ...metadata };
  }

  verify({
    analysisResult = null,
    sectionVerifier = this.sectionVerifier,
    includeLoadCases = this.includeLoadCases,
    limitStates = this.limitStates,
    verificationStations = this.verificationStations,
    context = {},
  }: BeamSectionActionVerifyInput = {}): VerificationResult {
    if (analysisResult == null) {
      throw new Error("BeamSectionActionVerifier requires an analysisResult.");
    }

    if (sectionVerifier == null) {
      throw new Error("BeamSectionActionVerifier requires a sectionVerifier.");
    }

    const verifySectionActions =
      typeof sectionVerifier === "function"
        ? sectionVerifier
        : sectionVerifier.verifySectionActions.bind(sectionVerifier);

    const requestedLimitStates =
      limitStates instanceof Set ? limitStates : normalizeLimitStates(limitStates);
    const entries = entriesFromResults(analysisResult, includeLoadCases).filter(
      (entry) =>
        requestedLimitStates === null ||
        requestedLimitStates.has(String(entry.context?.limitState ?? "").toUpperCase()),
    );
    const stationResults: NormalizedActionVerification[] = [];
    const checks: VerificationCheck[] = [];
    const warnings: string[] = [];
    const assumptions: string[] = [];

    for (const entry of entries) {
      const stationSettings = normalizeVerificationStationSettings(verificationStations, entry);
      const selection = selectVerificationSamples(entry, stationSettings);

      warnings.push(...selection.warnings);

      for (const { sample, stationMetadata } of selection.records) {
        const normalized = normalizeActionVerification(
          verifySectionActions({
            nEd: sample.n,
            vEd: sample.v,
            mEd: sample.m,
            tEd: sample.t ?? sample.tEd ?? sample.torsion ?? 0,
            principalActions: sample.principalActions ?? null,
            vYEd: sample.principalActions?.vY ?? sample.vY ?? null,
            vZEd: sample.principalActions?.vZ ?? sample.vZ ?? null,
            mYEd: sample.principalActions?.mY ?? sample.mY ?? null,
            mZEd: sample.principalActions?.mZ ?? sample.mZ ?? null,
            x: sample.station,
            station: sample.station,
            sample,
            context: {
              ...context,
              analysisResult,
              result: entry,
              resultId: entry.id,
              resultType: entry.resultType,
              limitState: entry.context?.limitState ?? null,
              combinationType: entry.context?.combinationType ?? null,
              stationMetadata,
              sectionProperties: entry.sectionProperties,
              units: entry.units ?? analysisResult.units,
            },
          }),
          {
            resultId: entry.id,
            resultType: entry.resultType,
            station: sample.station,
            limitState: entry.context?.limitState ?? null,
            principalActions:
              sample.principalActions != null ? { ...sample.principalActions } : null,
            ...stationMetadata,
          },
        );

        stationResults.push(normalized);
        warnings.push(...normalized.warnings);
        assumptions.push(...normalized.assumptions);
        checks.push(
          ...normalized.checks.map((check) => ({
            ...check,
            metadata: {
              resultId: entry.id,
              resultType: entry.resultType,
              station: sample.station,
              limitState: entry.context?.limitState ?? null,
              ...stationMetadata,
              ...((check.metadata as Record<string, unknown> | undefined) ?? {}),
            },
          })),
        );
      }
    }

    const governing = governingResult(stationResults);

    return new VerificationResult({
      applicationId: this.applicationId,
      status:
        stationResults.length > 0 &&
        stationResults.every((result) => result.status === RESULT_STATUS.OK)
          ? RESULT_STATUS.OK
          : RESULT_STATUS.NOT_VERIFIED,
      summary: "Section action verification along beam FEM samples.",
      utilizationRatio: governing?.utilizationRatio ?? null,
      demand: governing?.demand ?? null,
      capacity: governing?.capacity ?? null,
      checks,
      outputs: {
        stationResultCount: stationResults.length,
        checkCount: checks.length,
        governing:
          governing !== null
            ? {
                utilizationRatio: governing.utilizationRatio,
                demand: governing.demand,
                capacity: governing.capacity,
                metadata: { ...governing.metadata },
              }
            : null,
      },
      warnings: [...new Set(warnings)],
      assumptions: [...new Set(assumptions)],
      metadata: {
        ...this.metadata,
        resultCount: entries.length,
        limitStates: requestedLimitStates !== null ? [...requestedLimitStates] : null,
        verificationStations: copyVerificationStations(verificationStations),
      },
    });
  }
}

export function verifyBeamSectionActions(
  options: BeamSectionActionVerifierOptions & BeamSectionActionVerifyInput = {},
): VerificationResult {
  return new BeamSectionActionVerifier(options).verify(options);
}
