export interface SectionRotationInput {
  alpha?: number;
  angle?: number;
  value?: number;
  inputAlpha?: number;
...
export interface NormalizedSectionRotation extends Record<string, unknown> {
  alpha: number;
  units: "rad";
  convention: string;
  primaryAxis: string;
...
export interface BeamRotationProperties extends Record<string, unknown> {
...
} satisfies NormalizedSectionRotation);
...
function normalizeAngleUnits(units: unknown): string {
  const normalized = stringifyValue(units, "rad").trim().toLowerCase();
...
function angleToRadians(alpha: unknown, units: unknown): number {
  ...
  throw new Error(`Unsupported sectionRotation angle units: ${String(units)}.`);
}

export function normalizeSectionRotation(
  sectionRotation: number | SectionRotationInput | null = null,
  ...
  const inputUnits =
    sectionRotation.inputUnits ?? sectionRotation.units ?? sectionRotation.angleUnits ?? "rad";
  const inputAlpha =
...
export function sectionRotationFactors(
  sectionRotation: number | SectionRotationInput | null = null,
): { rotation: NormalizedSectionRotation; cos: number; sin: number } {
...
export function splitPrincipalActions(
  { n = 0, v = 0, m = 0 }: { n?: number; v?: number; m?: number } = {},
  sectionRotation: number | SectionRotationInput | null = null,
): {
  n: number;
  vY: number;
...
export function equivalentVerticalRigidity({
  ...
  value: number | null;
  source: string;
  rotation: NormalizedSectionRotation;
} {
  const { rotation, cos, sin } = sectionRotationFactors(sectionRotation);
...
export function applySectionRotationToBeamProperties({
  ...
  properties: BeamRotationProperties;
  sectionRotation?: number | SectionRotationInput | null | undefined;
  flexuralRigidityY?: number | null;
  ...
  return {
    ...properties,
    flexuralRigidity: flexural.value as number,
