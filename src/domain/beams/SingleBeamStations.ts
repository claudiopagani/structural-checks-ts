...
import {
  ...
  POINT_LOAD_TYPES,
  type BeamLoadInput,
  type BeamSupportDefinition,
} from "./SingleBeamInput.js";

export interface ResolvedBeamGeometry {
...
export function resolveGeometry(
  geometry: { start: { x: number; y: number }; end: { x: number; y: number } } | null | undefined,
  sourceUnits: UnitSystemInput,
  targetUnits: UnitSystemInput,
): ResolvedBeamGeometry {
...
export function coordinateAtStation(
...
function addStation(stations: number[], station: number | null, tolerance: number): void {
  if (station !== null && !stations.some((existing) => Math.abs(existing - station) <= tolerance)) {
    stations.push(station);
  }
}

function sortStations(stations: number[]): number[] {
...
export function resolveStation(
  ...
  const station = unitResolver.length(value as number);
...
function addDiscretizationStations(
  stations: number[],
  geometry: ResolvedBeamGeometry,
  unitResolver: UnitResolver,
  ...
  const maxElementLength =
    discretization.maxElementLength == null
      ? null
      : unitResolver.length(discretization.maxElementLength as number);
  const rawUserStations =
    discretization.stations ?? discretization.userStations ?? discretization.checkStations ?? [];
...
function normalizeVerificationStationMode(mode: unknown): string {
  const normalized = stringifyValue(mode, "combined").trim().toLowerCase();
...
function addVerificationStations(
  stations: number[],
  geometry: ResolvedBeamGeometry,
  unitResolver: UnitResolver,
  ...
  if (
    ...
    (typeof verificationStations === "object" &&
      !Array.isArray(verificationStations) &&
      verificationStations.enabled === false)
  ) {
    return;
  }

  const options: Record<string, unknown> = Array.isArray(verificationStations)
...
export function collectBeamStations({
  ...
  discretization?: Record<string, unknown>;
  verificationStations?: unknown[] | Record<string, unknown> | null;
  supports?: readonly BeamSupportDefinition[];
  loads?: readonly BeamLoadInput[];
  tolerance?: number;
}): number[] {
