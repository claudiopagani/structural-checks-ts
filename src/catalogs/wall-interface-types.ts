// Mechanical TypeScript migration from strutture-js 6f33baead8b88166c4b2cf94af41763412e3c751; source path: src/catalogs/wall-interface-types.js.
/* eslint-disable @typescript-eslint/ban-ts-comment, @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-argument, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-return, @typescript-eslint/no-unused-vars, @typescript-eslint/no-unnecessary-type-assertion */
// @ts-nocheck

import { SoilStructureInterface } from "../domain/geotechnics/SoilStructureInterface.js";
...
const surfaceEntries = [
  ...
  ["painted-steel-smooth", "Painted steel, smooth", "steel", "painted-smooth", "smooth"],
  ["rough-steel", "Rough steel", "steel", "rough", "rough-formed"],
  ["formed-concrete", "Formed concrete", "concrete", "formed", "rough-formed"],
  [
...
export const WALL_SURFACE_TYPE_CATALOG = Object.freeze(
  Object.fromEntries(
...
const soilEntries = [
  ...
  ["fine-sandy-silt", "Fine sandy silt to nonplastic silt", [0.075, 0.2], [14, 28, 28]],
];

interface USACE2022InterfaceRecommendationInput {
  wallSurfaceTypeId?: string;
  soilInterfaceClassId?: string;
}

interface SoilStructureInterfaceFromWallSurfaceInput {
  id?: string;
  name?: string | null;
  wallSurfaceTypeId?: string;
  soilInterfaceClassId?: string;
...
export const SOIL_INTERFACE_CLASS_CATALOG = Object.freeze({
  ...Object.fromEntries(
    soilEntries.map(([id, name, d50RangeMm, angles]) => [
      id,
      Object.freeze({
        id,
        name,
...
export const WALL_SURFACE_TYPE_IDS = Object.freeze(Object.keys(WALL_SURFACE_TYPE_CATALOG));
export const SOIL_INTERFACE_CLASS_IDS = Object.freeze(Object.keys(SOIL_INTERFACE_CLASS_CATALOG));
...
export function getUSACE2022InterfaceRecommendation({
...
}: USACE2022InterfaceRecommendationInput = {}) {
  const wallSurface = getWallSurfaceTypeData(wallSurfaceTypeId as string);
  const soilClass = getSoilInterfaceClassData(soilInterfaceClassId as string);
  ...
  return {
    ...
    value,
    angleUnits: soilClass.recommendationModel === "assigned-angle" ? "deg" : null,
    basis: "indicative",
    provenance: {
...
export function createSoilStructureInterfaceFromWallSurface({
  ...
  metadata = {},
}: SoilStructureInterfaceFromWallSurfaceInput = {}) {
  const wallSurface = getWallSurfaceTypeData(wallSurfaceTypeId as string);
  const recommendation = getUSACE2022InterfaceRecommendation({
    wallSurfaceTypeId: wallSurfaceTypeId as string,
    soilInterfaceClassId: soilInterfaceClassId as string,
  });
  const parameterSet = {
    id: parameterSetId,
