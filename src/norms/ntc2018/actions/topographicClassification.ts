...
// source path: src/norms/ntc2018/actions/topographicClassification.js.
...
import {
  ...
  TPI_THRESHOLD_M,
  WORKING_CELL_SIZE_M,
  type TopographicClassificationMode,
  type TopographicPreprocessingMode,
} from "./topographicClassification.constants.js";

export type NullableNumberMatrix = Array<Array<number | null>>;
export type BooleanMatrix = boolean[][];

export interface TopographicModelCell {
  row: number;
  col: number;
  x: number;
  y: number;
...
export interface TopographicModel {
  ...
  yMin: number;
  yMax: number;
  cells: TopographicModelCell[];
  cellMap: ReadonlyMap<string, TopographicModelCell>;
}

export interface TopographicSlopeDirection {
  ratio: number;
  direction: { x: number; y: number };
  neighborOffset: { row: number; col: number; x: number; y: number };
  elevationDeltaM: number;
  distanceM: number;
}

export interface TopographicClassificationResult {
  class: string | null;
  category: string | null;
  amplificationFactorST: number | null;
  slopeDeg: number | null;
  slopePercent: number | null;
  slopeDirection: TopographicSlopeDirection | null;
  slopeMethod: string;
  tpiCenterM: number | null;
  isRidge: boolean;
  h30Center: boolean | null;
  h60Center: boolean | null;
  localElevationRangeM: number | null;
  ridgeAtCenter: boolean;
  mode: string | null;
  preprocessingMode: string | null;
  preprocessingLabel: string | null;
  isArticleCompliantScale: boolean;
  isArticleCompliantPreprocessing: boolean;
  isArticleCompliant: boolean;
  isReliable: boolean;
  warnings: string[];
...
type TopographicClass = keyof typeof TOPOGRAPHIC_AMPLIFICATION_FACTORS;

interface SlopeAnalysisMap {
  slopes: NullableNumberMatrix;
  ratios: NullableNumberMatrix;
  directions: Array<Array<TopographicSlopeDirection | null>>;
}

interface TpiMap {
  tpi: NullableNumberMatrix;
  coverage: number[][];
  coverageWarnings: Array<{ row: number; col: number; coverageRatio: number }>;
}

interface ElevationRangeMap {
  ranges: NullableNumberMatrix;
  h30: Array<Array<boolean | null>>;
  h60: Array<Array<boolean | null>>;
}

interface RidgeMap {
  ridgeZone: BooleanMatrix;
  ridgeLine: BooleanMatrix;
  candidateRidge: BooleanMatrix;
  filteredRidge: BooleanMatrix;
  ridgeMap: BooleanMatrix;
}
...
function getModeForGridSize(gridSize: number): TopographicClassificationMode | null {
  return (
    Object.values(TOPOGRAPHIC_CLASSIFICATION_MODES).find((mode) => mode.gridSize === gridSize) ??
    null
  );
}

function getPreprocessingMode(modeName: unknown): TopographicPreprocessingMode {
  const mode = typeof modeName === "string" ? TOPOGRAPHIC_PREPROCESSING_MODES[modeName] : undefined;
  const fallback = TOPOGRAPHIC_PREPROCESSING_MODES[DEFAULT_TOPOGRAPHIC_PREPROCESSING_MODE];
  if (mode !== undefined) {
    return mode;
  }
  if (fallback === undefined) {
    throw new Error("The default topographic preprocessing mode is unavailable.");
  }
  return fallback;
}

function formatMissingRatio(ratio: number): string {
...
function getCell(model: TopographicModel, row: number, col: number): TopographicModelCell | null {
  return model.cellMap?.get(`${row}:${col}`) ?? null;
}

function validateModel(model: TopographicModel | null | undefined): ModelValidation {
...
function buildRawElevationMatrix(model: TopographicModel): NullableNumberMatrix {
  const matrix = createMatrix<number | null>(model.gridSize, model.gridSize, null);

  for (let row = 0; row < model.gridSize; row += 1) {
    for (let col = 0; col < model.gridSize; col += 1) {
      const cell = getCell(model, row, col);
      matrix[row]![col] =
        cell?.isNoData === true || cell?.rawElevation == null ? null : Number(cell.rawElevation);
    }
  }
...
function interpolateRawElevation(
  model: TopographicModel,
  matrix: NullableNumberMatrix,
  x: number,
  y: number,
): number | null {
...
function buildWorkingGrid(
  model: TopographicModel,
  rawMatrix: NullableNumberMatrix,
  cellSizeM = WORKING_CELL_SIZE_M,
): {
  elevations: NullableNumberMatrix;
  coordinates: Array<Array<MatrixPoint | null>>;
  size: number;
  ...
  const elevations = createMatrix<number | null>(size, size, null);
  const coordinates = createMatrix<MatrixPoint | null>(size, size, null);
  ...
  return { elevations, coordinates, size, maxCoordinate, cellSizeM };
}

export function smoothWorkingGrid(
  elevations: NullableNumberMatrix,
  windowCells = SMOOTHING_WINDOW_CELLS,
): { elevations: NullableNumberMatrix; reliable: BooleanMatrix } {
  ...
  const smoothed = createMatrix<number | null>(rows, cols, null);
  const reliable = createMatrix<boolean>(rows, cols, false);
  ...
  return { elevations: smoothed, reliable };
}

export function computeSlopeAnalysisMap(
  dem: NullableNumberMatrix,
  cellSizeM = WORKING_CELL_SIZE_M,
): SlopeAnalysisMap {
  const rows = dem.length;
  const cols = dem[0]?.length ?? 0;
  const slopes = createMatrix<number | null>(rows, cols, null);
  const directions = createMatrix<TopographicSlopeDirection | null>(rows, cols, null);
  const ratios = createMatrix<number | null>(rows, cols, null);
  ...
  for (let row = 1; row < rows - 1; row += 1) {
    for (let col = 1; col < cols - 1; col += 1) {
      ...
      let maxNeighbor: TopographicSlopeDirection | null = null;
      ...
      for (const { dr, dc } of neighborOffsets) {
        ...
        if (!maxNeighbor || absoluteRatio > maxNeighbor.ratio) {
          ...
          maxNeighbor = {
            ...
            neighborOffset: { row: dr, col: dc, x: xOffsetM, y: yOffsetM },
            elevationDeltaM,
            distanceM,
          };
        }
  ...
  return { slopes, ratios, directions };
}

export function computeSlopeMap(
  dem: NullableNumberMatrix,
  cellSizeM = WORKING_CELL_SIZE_M,
): NullableNumberMatrix {
  return computeSlopeAnalysisMap(dem, cellSizeM).slopes;
}

function getTheoreticalCircleOffsets(
  radiusM: number,
  cellSizeM: number,
...
export function computeTpiMap(
  dem40Smooth: NullableNumberMatrix,
  cellSizeM = WORKING_CELL_SIZE_M,
  radiusM = TPI_RADIUS_ARTICLE_M,
): TpiMap {
  const rows = dem40Smooth.length;
  const cols = dem40Smooth[0]?.length ?? 0;
  const tpi = createMatrix<number | null>(rows, cols, null);
  const coverage = createMatrix<number>(rows, cols, 0);
  const offsets = getTheoreticalCircleOffsets(radiusM, cellSizeM);
  const expectedCount = offsets.length;
  const coverageWarnings: Array<{ row: number; col: number; coverageRatio: number }> = [];
  ...
  for (let row = 0; row < rows; row += 1) {
    for (let col = 0; col < cols; col += 1) {
      ...
      if (coverageRatio < 0.75 || availableCount === 0) {
  ...
  return { tpi, coverage, coverageWarnings };
}
...
export function computeElevationRangeMap(
  ...
  const rows = dem.length;
  const cols = dem[0]?.length ?? 0;
  const ranges = createMatrix<number | null>(rows, cols, null);
  const h30 = createMatrix<boolean | null>(rows, cols, null);
  const h60 = createMatrix<boolean | null>(rows, cols, null);
  const halfWindow = Math.floor(windowCells / 2);
  ...
  return { ranges, h30, h60 };
}

function buildRidgeZone(tpiMap: NullableNumberMatrix): BooleanMatrix {
  return tpiMap.map((row) => row.map((value) => value != null && value > TPI_THRESHOLD_M));
}

function countBinaryNeighbors(
  matrix: BooleanMatrix,
  row: number,
  col: number,
): { neighbors: number[]; count: number; transitions: number } {
  const neighbors = [
  ...
  return { neighbors, count, transitions };
}

export function zhangSuenThinning(binaryMatrix: BooleanMatrix): BooleanMatrix {
    ...
    for (const step of [0, 1]) {
      ...
      for (let row = 1; row < rows - 1; row += 1) {
        for (let col = 1; col < cols - 1; col += 1) {
          ...
          const { neighbors, count, transitions } = countBinaryNeighbors(output, row, col);
          const p2 = neighbors[0] ?? 0;
          const p4 = neighbors[2] ?? 0;
          const p6 = neighbors[4] ?? 0;
          const p8 = neighbors[6] ?? 0;
...
function intersectRidgeWithH30(
  ridgeLine: BooleanMatrix,
  h30Map: Array<Array<boolean | null>>,
): BooleanMatrix {
...
function filterSmallComponents(binaryMatrix: BooleanMatrix, minCells: number): BooleanMatrix {
  const rows = binaryMatrix.length;
  const cols = binaryMatrix[0]?.length ?? 0;
  const output = createMatrix<boolean>(rows, cols, false);
...
function dilateChebyshev(binaryMatrix: BooleanMatrix, radiusCells: number): BooleanMatrix {
  const rows = binaryMatrix.length;
  const cols = binaryMatrix[0]?.length ?? 0;
  const output = createMatrix<boolean>(rows, cols, false);
...
export function buildRidgeMap(
  ...
  const filteredRidge = filterSmallComponents(candidateRidge, minConnectedCells);
  const ridgeMap = dilateChebyshev(filteredRidge, dilationCells);

  return { ridgeZone, ridgeLine, candidateRidge, filteredRidge, ridgeMap };
}

function addThresholdWarnings(
  warnings: string[],
  result: Pick<TopographicClassificationResult, "slopeDeg" | "tpiCenterM" | "localElevationRangeM">,
): void {
  if (
    typeof result.slopeDeg === "number" &&
    Number.isFinite(result.slopeDeg) &&
    Math.abs(result.slopeDeg - SLOPE_THRESHOLD_DEG) < 2
  ) {
    addWarning(
      warnings,
  ...
  if (
    typeof result.tpiCenterM === "number" &&
    Number.isFinite(result.tpiCenterM) &&
    Math.abs(result.tpiCenterM - TPI_THRESHOLD_M) < 2
  ) {
    addWarning(
      warnings,
  ...
  if (
    typeof result.localElevationRangeM === "number" &&
...
function buildEmptyResult(
  model: TopographicModel | null | undefined,
  mode: TopographicClassificationMode | null,
  preprocessingMode: TopographicPreprocessingMode,
  warnings: string[],
): TopographicClassificationResult {
  return {
    class: null,
    category: null,
    ...
    isArticleCompliantPreprocessing: preprocessingMode?.isArticleCompliantPreprocessing === true,
    isArticleCompliant:
    ...
    diagnostics: {
      ...
      ridgeDilationCells: preprocessingMode?.ridgeDilationCells ?? RIDGE_DILATION_CELLS_40M,
      ridgeDilationM: preprocessingMode?.ridgeDilationM ?? RIDGE_DILATION_M,
    },
  };
}

export default function classifyTopographyMascandola(
  model: TopographicModel | null | undefined,
  ...
  if (model == null || !validation.canClassify || !mode) {
    return buildEmptyResult(model, mode, preprocessingMode, warnings);
  }

  const rawMatrix = buildRawElevationMatrix(model);
  const workingGrid = buildWorkingGrid(model, rawMatrix, preprocessingMode.workingCellSizeM);
  ...
  const tpi = computeTpiMap(processed.elevations, preprocessingMode.workingCellSizeM, mode.radiusM);
  ...
  const centerSlopeRatio = slopeAnalysis.ratios[centerIndex]?.[centerIndex] ?? null;
  const centerSlopeDirection = slopeAnalysis.directions[centerIndex]?.[centerIndex] ?? null;
  ...
  const localElevationRangeM = elevationRanges.ranges[centerIndex]?.[centerIndex] ?? null;
  ...
  let topographicClass: TopographicClass | null = null;
  ...
  const result: TopographicClassificationResult = {
    class: topographicClass,
    category: topographicClass,
    amplificationFactorST:
      topographicClass == null ? null : TOPOGRAPHIC_AMPLIFICATION_FACTORS[topographicClass],
    slopeDeg: centerSlopeDeg,
    slopePercent: Number.isFinite(centerSlopeRatio) ? Number(centerSlopeRatio) * 100 : null,
    ...
    isArticleCompliant:
      mode.isArticleCompliantScale && preprocessingMode.isArticleCompliantPreprocessing,
  ...
  if (topographicClass === "T3" || topographicClass === "T4") {
    addWarning(
      warnings,
