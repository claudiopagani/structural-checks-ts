...
// source path: src/norms/ntc2018/actions/topographicClassification.constants.js.
...
export const TOPOGRAPHIC_AMPLIFICATION_FACTORS = Object.freeze({
  T1: 1.0,
  T2: 1.2,
  T3: 1.2,
  T4: 1.4,
...
export interface TopographicClassificationMode {
  mode: string;
  radiusM: number;
  gridSize: number;
  label: string;
...
export const TOPOGRAPHIC_CLASSIFICATION_MODES: Readonly<
  Record<string, TopographicClassificationMode>
> = Object.freeze({
  "full-article": {
    mode: "full-article",
    radiusM: 500,
...
export interface TopographicPreprocessingMode {
  mode: string;
  label: string;
  description: string;
  workingCellSizeM: number;
  smoothingWindowCells: number;
  hWindowCells: number;
  ridgeDilationCells: number;
  ridgeDilationM: number;
  ridgeMinConnectedCells: number;
  isArticleCompliantPreprocessing: boolean;
}

export const TOPOGRAPHIC_PREPROCESSING_MODES: Readonly<
  Record<string, TopographicPreprocessingMode>
> = Object.freeze({
