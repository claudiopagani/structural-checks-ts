...
// Source path: src/norms/ntc2018/materials/ntc2018ExistingMasonryCatalogs.js.

export interface Ntc2018ExistingMasonryTypology {
  id: number;
  name: string;
  notes: string | null;
...
export interface Ntc2018ExistingMasonryModifierDefinition {
  id: number;
  key: string;
  label: string;
  phase: "survey" | "improvement";
  incompatibleWith?: number[];
  usesTypologyValueKey?: string;
}

export interface Ntc2018ExistingMasonryParameterLevel {
  id: number;
  label: string;
  description: string;
}

export type Ntc2018ExistingMasonryParameterLevels = Record<
  1 | 2,
  Ntc2018ExistingMasonryParameterLevel
>;

export type Ntc2018ExistingMasonryMechanicalProperties = Record<string, number | undefined>;

type Ntc2018ExistingMasonryTypologyInput = Omit<Ntc2018ExistingMasonryTypology, "notes"> & {
  notes?: string | null;
};
...
const createRangeValue = (values: number[], level: number): number | undefined => {
...
const createTypology = ({
  ...
  ranges,
  multipliers,
}: Ntc2018ExistingMasonryTypologyInput): Ntc2018ExistingMasonryTypology => ({
  id,
  name,
  notes,
  ranges,
...
export const NTC2018_EXISTING_MASONRY_TYPOLOGIES: Ntc2018ExistingMasonryTypology[] = [
  createTypology({
    id: 1,
...
export const NTC2018_EXISTING_MASONRY_MODIFIER_DEFINITIONS: Ntc2018ExistingMasonryModifierDefinition[] =
  [
    {
      id: 1,
      key: "maltaBuona",
...
export const NTC2018_EXISTING_MASONRY_PARAMETER_LEVELS: Ntc2018ExistingMasonryParameterLevels = {
  1: {
    id: 1,
    label: "livello 1",
...
export function resolveMasonryTypology(
  input: unknown,
): Ntc2018ExistingMasonryTypology | null | undefined {
  ...
  return NTC2018_EXISTING_MASONRY_TYPOLOGIES.find((item) => item.name.toLowerCase() === normalized);
}

export function getTabulatedMechanicalProperties(
  typology: Ntc2018ExistingMasonryTypology,
  parameterLevel: number,
): Ntc2018ExistingMasonryMechanicalProperties {
  return Object.entries(typology.ranges).reduce<Ntc2018ExistingMasonryMechanicalProperties>(
    (accumulator, [key, values]) => {
      accumulator[key] = key === "w" ? average(values) : createRangeValue(values, parameterLevel);
      return accumulator;
    },
    {},
  );
