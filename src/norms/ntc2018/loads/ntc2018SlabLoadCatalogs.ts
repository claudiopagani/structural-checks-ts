...
// Source path: src/norms/ntc2018/loads/ntc2018SlabLoadCatalogs.js.
...
import { VariableLoad, type VariableLoadJson } from "../../../domain/slabs/VariableLoad.js";
...
export interface SlabMaterialWeightPresetEntry {
  description: string;
  value: number;
}

export interface SlabMaterialWeightPresetGroup {
  category: string;
  entries: SlabMaterialWeightPresetEntry[];
}

export interface SlabMaterialWeightPresetDatabase {
  [weightType: string]: SlabMaterialWeightPresetGroup[];
  volumeWeights: SlabMaterialWeightPresetGroup[];
  surfaceWeights: SlabMaterialWeightPresetGroup[];
  lineWeights: SlabMaterialWeightPresetGroup[];
}

export const SLAB_MATERIAL_WEIGHT_PRESET_DATABASE: SlabMaterialWeightPresetDatabase = {
  volumeWeights: [
    {
      ...
      entries: [
        ...
        {
          description: "Calcestruzzo armato con densita media (ca. 1-2% di armatura)",
          value: 25.0,
        },
    ...
    },
    {
      ...
      entries: [
        ...
        {
          description: "Muratura in blocchi di laterizio forato (es. forati da 800 kg/m3)",
          value: 8.0,
        },
  ...
  surfaceWeights: [
    ...
    },
    {
      ...
      entries: [
        {
          description: "Controsoffitto in lastre di cartongesso con orditura metallica",
          value: 0.15,
        },
    ...
    },
    {
      ...
      entries: [
        {
          description:
            "Incidenza impianti elettrici e idraulici a pavimento (uso residenziale/uffici)",
          value: 0.3,
        },
        { description: "Incidenza impianti complessi (commerciale/industriale)", value: 0.5 },
        {
          description: "Incidenza peso pannelli fotovoltaici con struttura di supporto",
          value: 0.2,
        },
      ],
    },
 ...
 */
export const NTC2018_SLAB_MATERIAL_WEIGHT_DATABASE = SLAB_MATERIAL_WEIGHT_PRESET_DATABASE;

interface LegacySlabVariableActionDescription {
  id: number;
  category: string;
  subcategory: string;
  description: string;
...
export interface NTC2018SlabVariableAction {
  id: number;
  category: string;
  subcategory: string;
  description: string;
  ...
  Hk: number | null;
  qkMode: string;
  qkMinimum: number | null;
  requiresGenericResolution: boolean;
  deprecated: true;
  replacement: string;
}

const LEGACY_SLAB_VARIABLE_ACTION_DESCRIPTIONS: LegacySlabVariableActionDescription[] = [
  {
    id: 1,
    category: "A",
    subcategory: "A",
...
function legacyQuantityValue(specification: { mode: string; value?: number }): number | null {
  return specification.mode === "fixed" ? (specification.value ?? null) : null;
}

function hasMinimum(value: object): value is { minimum?: unknown } {
  return "minimum" in value;
}

function legacyMinimumValue(specification: unknown): number | null {
  if (specification !== null && typeof specification === "object" && hasMinimum(specification)) {
    return typeof specification.minimum === "number" ? specification.minimum : null;
  }

  return null;
}

export const NTC2018_SLAB_VARIABLE_ACTIONS_DATABASE: readonly NTC2018SlabVariableAction[] =
  Object.freeze(
    LEGACY_SLAB_VARIABLE_ACTION_DESCRIPTIONS.map((legacyEntry) => {
      ...
      if (!definition) {
        throw new Error(
          `Missing generic imposed-load definition for legacy slab action ${legacyEntry.id}.`,
        );
      }
      ...
      return Object.freeze({
        ...
        qkMinimum: legacyMinimumValue(definition.loads.qk),
...
export interface VariableLoadDocumentation {
  reference?: unknown;
  [key: string]: unknown;
}

export interface CreateNTC2018SlabVariableLoadOptions {
  actionId: number;
  description?: string | null;
  qk?: number | null;
  documentation?: VariableLoadDocumentation | null;
  units?: UnitSystemInput | null;
}

export function listSlabMaterialWeightPresetCategories(weightType: string): string[] {
...
export function listSlabMaterialWeightPresetEntries(
  weightType: string,
  category: string,
...
export interface GetSlabMaterialWeightPresetValueOptions {
  weightType: string;
  category: string;
  description: string;
}
...
export function getSlabMaterialWeightPresetValue({
  ...
  category,
  description,
}: GetSlabMaterialWeightPresetValueOptions): number {
  const entry = listSlabMaterialWeightPresetEntries(weightType, category).find(
...
export function listNTC2018SlabWeightCategories(weightType: string): string[] {
  return listSlabMaterialWeightPresetCategories(weightType);
}

/** @deprecated Use listSlabMaterialWeightPresetEntries. */
export function listNTC2018SlabWeightEntries(
  weightType: string,
  category: string,
): SlabMaterialWeightPresetEntry[] {
...
export function getNTC2018SlabWeightValue(input: GetSlabMaterialWeightPresetValueOptions): number {
  return getSlabMaterialWeightPresetValue(input);
}

export function getNTC2018SlabVariableAction(actionId: number): NTC2018SlabVariableAction {
...
export function createNTC2018SlabVariableLoad({
  ...
  documentation = null,
  units = null,
}: CreateNTC2018SlabVariableLoadOptions): VariableLoad {
  if (units == null) {
  ...
  if (factors === undefined) {
    throw new Error(`Missing NTC 2018 combination factors for category: ${action.category}.`);
  }
  ...
  if (minimum != null && load.value < minimum) {
    throw new Error(`qk for ${action.definitionId} must not be lower than ${minimum} kN/m^2.`);
  }
...
export type NTC2018SlabVariableLoadJson = VariableLoadJson;
