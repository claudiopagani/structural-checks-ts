/* eslint-disable @typescript-eslint/ban-ts-comment, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-return, @typescript-eslint/no-floating-promises */
...
test("global FEM units must be complete and unambiguous", () => {
  ...
  assert.ok(
    ambiguousValidation.errors.some(
...
test("line and shell local axes must be complete, orthonormal and non-degenerate", () => {
  ...
  assert.ok(
    incompleteValidation.errors.some((item) => item.path === "$.lineElements[0].localAxes.z"),
  );
...
test("requested results cannot be omitted when the capability is declared", () => {
  ...
  assert.ok(
    validation.errors.some(
      (item) => item.code === "FEM_DECLARED_RESULT_MISSING" && item.path === "$.results.modes",
    ),
  );
});
...
test("element-average shell resultants declare their averaging provenance", () => {
  ...
  assert.ok(missingProvenance.errors.some((item) => item.path.endsWith(".location.averaging")));
  ...
  for (const [field, value] of [
    ...
    assert.ok(invalid.errors.some((item) => item.path.endsWith(`.location.averaging.${field}`)));
  }
});
...
test("individual result validation may omit mapping while the complete set requires it", () => {
  ...
  assert.ok(
    completeSet.contracts.mapping.errors.some(
...
test("punching mapping connects one slab to supporting member ends", () => {
  const fixture = createGlobalFemBuildingFixture();
  ...
  const valid = validateFemEntityMappingContract(fixture.mapping, {
    model: fixture.model,
  });

  assert.equal(valid.ok, true, JSON.stringify(valid.errors));

  fixture.mapping.punchingConnections[0].supportLineElementEnds[0].end = "start";
  const invalid = validateFemEntityMappingContract(fixture.mapping, {
    model: fixture.model,
  });
  assert.equal(invalid.ok, false);
  assert.ok(diagnosticCodes(invalid).includes("FEM_JOINT_END_MISMATCH"));
});
