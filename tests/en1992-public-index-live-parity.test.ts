...
void test("EN 1992 public index matches the independent pinned JavaScript implementation", async () => {
  assert.equal(gitOutput("rev-parse", "HEAD"), sourceRevision);
  assert.equal(gitOutput("status", "--porcelain"), "");

  const sourceIndex = await loadModule(sourceRoot, "src/norms/en1992/index.js");
  const typescriptIndex = await loadModule(
    path.join(repositoryRoot, "dist"),
    "norms/en1992/index.js",
  );
  const sourceKeys = Object.keys(sourceIndex);
  assert.deepEqual(Object.keys(typescriptIndex), sourceKeys, "exact EN 1992 export order");
